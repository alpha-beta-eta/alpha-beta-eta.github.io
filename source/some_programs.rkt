...
(define (zero-array A)
  (match A
    ((array shape _) (array shape (const 0)))))
(define (transpose A)
  (match A
    ((array (list i j) indexf)
     (array (list j i)
...
(define ((normalize bound) A)
  (match-define (cons min max) (min-max A))
            ...
            (λ indices
              (apply indexf
                     (clamp (list- indices s)
                            shape)))))))
(define ((convolve2 A center) pattern)
  (&lt;&lt; pattern
      (Mapi
       (λ (n . indices)
         (if (zero? n)
