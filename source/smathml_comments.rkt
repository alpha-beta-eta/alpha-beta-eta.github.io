...
(define (|(]| a b)
  ...
  (cond ((null? lst) '())
   ...
   (H2 "关于数学自动转换")
   (P "目前在数学上下文中, 符号, 数字, "
      "字符串都能自动转换为符合MathML标准的形式. "
      "在数学上下文之外, 符号和数字也是如此. "
      "这当然还预留了很大的空间, "
      "不过暂时还没有利用. 比如说, "
      "我们可以让向量类型去包裹某种符号表达式, "
      "然后再将这种符号表达式转换为合法的MathML形式.")
   (P "问题来了, 既然这种方案看上去非常合理, "
      "为什么我不这么做呢? "
      "这可能是因为我在很久以前尝试设计SMathML的时候, "
      "就完全想的是将符号表达式转换为MathML的方案. "
      "但是问题在于, 它需要ad hoc地处理大量琐碎的细节问题, "
      "最后只能对于很简单的情况有效. "
      "我目前还没看到提供这种机制能够带来什么实际效用, "
      "所以暂时就不准备做了. 当然, "
      "一旦我能想到什么绝妙的方案, "
      "还是会进行试验的, 然而不是现在.")
   (H2 "关于Skribilo")
   (P "最近我发现Skribilo在很多设计和实现思路和SMathML都很像, "
      "不过实际上也作出了不少迥异的设计决策, "
      "以至于实际使用体验完全不同. "
      "Skribilo在数学方面的体验有一部分和SMathML相同, "
      "因为用户都可以使用各种预定义的过程将数学表达式拼在一起. "
      "不过, 似乎Skribilo在此方面的可扩展性不好. "
      "另外, Skribilo更倾向于让用户写S-exp (Scheme列表), "
      "然后再将其转换为真正的数学表达式. "
      "这种设计决策天生在可扩展性方面就需要更多努力和思考.")
