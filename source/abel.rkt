...
(define abel.html
  (TnTmPrelude
   ...
   (H2 "引论")
   (P "本书我们从研究求解具有一个未知元的从一次到四次的代数方程的问题开始. "
      "古代数学家已经明白该如何求解一次和二次的代数方程, "
      "但是求解三次和四次代数方程的方法到了十六世纪才建立. "
      "具有如下形式的方程"
      (MB (&= (&+ (&i* $a_0 $x^n) (&i* $a_1 (^ $x (&- $n $1))) $..c
                  (&i* (_ $a (&- $n $1)) $x) $a_n) $0))
      "其中" (&!= $a_0 $0) ", 被称为具有一个未知元的"
      $n "次一般代数方程.")
   (P "令" (&= $n $1) "我们就得到了线性方程"
   ...
   ((proof)
    "考虑映射" (func:def $phi (&c* $G $H) (&c* $H $G) (tu0 $g $h) (tu0 $h $g))
    ", 这是显然的同构.")
   ((exercise #:n "73")
   ...
   ((exercise #:n "94")
    "证明内自同构是一个群到自身的同构.")
   ((proof)
    "对于群" $G "和" (∈ $g $G) ", 考虑内自同构" (func:def $phi_g $G $G $h (&i* $g $h (inv $g)))
