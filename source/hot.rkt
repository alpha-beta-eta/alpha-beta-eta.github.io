...
(define $cell (Miv "cell" "sans-serif"))
(define (&cell a v)
  (&split2 $cell a v))
(define (&type id de)
  (&split2 $type (&= id de)))
...
(define hot.html
  (TnTmPrelude
   ...
   (P "到目前为止我们知道了我们如何表示大值, "
      ...
      "不能存放具有任意大小的值, "
      "因而我们需要将大值分解为小的部分. "
      "我们的方法是使用一种抽象语义对象" (&cell $a $v)
      ", 其表示地址" $a "处的内存单元存放着小值" $v
      ". 一个小值要么是一对地址, 要么是单位值, "
      "要么是一个标签连带着一个地址. "
      "例如, 之前的例子里的数字二可以表示为"
      (MB (set-attr*
           (&Table
            ...
            ($cell $4 (tu0)))
           'columnalign "left"))
      "这里我们用了整数来表示地址, 这对于真实内存而言实际上是典型的. "
      "我们也假定每个内存单元都有足够的空间用于存放任意的小值. "
