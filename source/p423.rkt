#lang racket
(provide thunk:p423.html)
(require SMathML)
(define (thunk:p423.html)
  (TmPrelude
   #:title "P423笔记"
   #:css "styles.css"
   (H1 "P423笔记")
...
'(let ((t.3 (cons '0 (cons '1 (cons '2 '()))))
  ...
  (even? 88))"))
   (P "现在让我们来详细描述作业14里的" (Code "purify-letrec")
      ...
      "那么它就是一个复杂绑定. "
      (S "(其实这里我有个小小的疑问, 就是如果" (Code "letrec")
         "的右支表达式引用了其绑定的变量, 然而却没有被" (Code "lambda")
         "包裹, 那么此时其实编译器按照" (Code "letrec") "的语义应该引起一个异常才对. "
         "(当然, 被" (Code "lambda") "包裹还是有可能被求值, 此时还是应该引起异常.) "
         "换言之, 一个合理的" (Code "letrec") "表达式的右支出现的没有包裹在"
         (Code "lambda") "里的变量一定是不会引用该" (Code "letrec") "绑定的变量的. "
         "我的猜测是, 这留待运行时才会抛出异常, 因为全然分析这种可能性应该也是不可计算的. "
         "不过, 在简单的情形下, 检查出这种错误应该是容易的, 比如上述情况.)"))
   (P "最终, 在我们将" (Code "letrec") "的绑定划分为了"
      (CodeB "(x_s e_s) ... : simple
(x_l e_l) ... : lambda
(x_c e_c) ... : complex")
      "之后, 我们所期望的变换是"
      (CodeB "(let ((x_s e_s) ...)
  (assigned ()
    (let ((x_c (void)) ...)
      ...
      ", 这是因为" (Code "letrec")
      "所绑定的每个变量均不会被赋值.")
   (CodeB "(define (purify-letrec exp)
  (define (purify exp)
    (match exp
      ((quote ,i) exp)
   ...
   (P "以上这些代码对于" (Code "purify-letrec")
      "而言都不是实质性的, 所以让我们将注意力聚焦到对于"
      (Code "letrec") "的处理上来.")
   (CodeB "      ((letrec ,bds (assigned ,u* ,body))
       (: bds
          (lambda (x* e*)
            (let ((e* (map purify e*))
                  (body (purify body)))
              ...
              (define (Leta x* e* u* body)
                (if (null? x*)
                    ...
                    `(let ,(map list x* e*)
                       (assigned ,u* ,body))))")
   (P "首先, 我们应该做一些准备工作, 故我们递归地变换了"
      (Code "e*") "和" (Code "body") ", 并定义了"
      (Code "assigned?") ", " (Code "bound?")
      ", " (Code "lambda?") ", " (Code "simple?")
      ", " (Code "Leta") ". 其中的" (Code "Leta")
      "只是为了方便地构造带有" (Code "assigned")
      "的" (Code "let") "而已, 不过它还多做了一个判断, "
      "避免空" (Code "let") "绑定的产生.")
   (CodeB "              (let iter ((x* x*)
                         (e* e*)
                         (xs* '())
                         ...
                         (Leta
                          ...
                          `(begin
                             ,@(map (lambda (xc xt)
                                    ...
                                    xc* xt*)
                             ,body))))))")
   (P "这部分展示了我们通过一个迭代将绑定划分为不同的种类, "
      "并且在划分结束后按照之前所描述的转换方式来构造表达式.")
   (CodeB "                    (let ((x (car x*))
                          (x* (cdr x*))
                          (e (car e*))
                          (e* (cdr e*)))
                      (cond ((assigned? x)
                            ...
                            (else
                             (iter x* e* xs* es* xl* el*
                                   (cons x xc*) (cons e ec*)))))))))))")
   (P "这部分是划分的逻辑: 如果变量被赋值, 那么不论如何绑定都是复杂的; "
      "接着表达式有可能是lambda, 那么绑定就是lambda; "
      "然后表达式有可能是简单的, 那么绑定就是简单的; "
      "最后, 如果还没有被归类, 那无疑就是一个复杂绑定了.")
   (CodeB "      ((,prim . ,rands)
       (guard (prim? prim))
  ...
  (purify exp))")
   ...
   (CodeB "&lt;exp> ::= (quote &lt;immediate>)
       ...
       |  (&lt;prim> &lt;exp>*)
       |  (&lt;exp> &lt;exp>*)")
   (P "最后, 我们给出" (Code "purify-letrec")
      "输出的语言的句法. 我们可以看到, " (Code "letrec")
      "不再具有" (Code "assigned") "形式, 并且其右支的诸表达式一定是"
      (Code "lambda") "表达式.")
   (P "以下是一些例子.")
   (CodeB "> (compil
   '(letrec ((even? (lambda (n)
...
'(let ((f.0 (void)))
   ...
   (P "我们没有试图消除冗余的" (Code "begin")
      ", 不过这在之后的pass会被优化, 空" (Code "let")
      "和" (Code "letrec") "也是.")
   (H2 "作业14: " (Code "convert-assignments"))
   (P (Code "convert-assignments") "施行的是所谓的赋值变换 (assignment conversion). "
      "据我所知, 这是Kent Dybvig发明的, 用来解决闭包变换和赋值不能兼容的问题. "
      "这个变换会降低具有大量赋值的程序的运行速度, 但是正常的Scheme程序应该也没有多少赋值.")
   (P "目前, " (Code "let") "和" (Code "lambda") "还具有附加的" (Code "assigned")
      "结构, 这将作为我们的脚手架使用. 我们会用" (Code "box") "包裹被赋值变量的值, 将"
      (Code "set!") "形式转变为对于函数" (Code "set-box!") "的调用, 将对于被赋值变量的引用"
      "转变为通过调用" (Code "unbox") "函数得到置于盒子 (box) 中的值.")
   (P "举一个简单的例子或许比较容易理解."
      (CodeB "(let ((x '2) (y '3))
  (assigned (x)
    (begin
  ...
  (let ((x (box t)))
    (begin
      (set-box! x (* (unbox x) y))
      (unbox x))))")
      "对于" (Code "lambda") "的处理也是类似的.")
   (P (Code "convert-assignments") "应该是相当简单的pass. "
      "注意到我们应该维护一个运行时的上下文, "
      "其记录了哪些变量是被赋值了的.")
   (CodeB "(define (convert-assignments exp)
  (define (Let x* e* body)
    (if (null? x*)
        body
  ...
  (define (convert env)
   ...
   (P "开头没有什么希奇的地方. 这里的" (Code "Let")
      "是为了避免冗余的" (Code "let") "绑定, 这是因为每当我们遇到"
      (Code "lambda") "和" (Code "let") "时, 都要试图转换"
      (Code "assigned") "结构, 而对于空的" (Code "assigned")
      ", 没有必要放置一个空的" (Code "let") "绑定上去. 另外, "
      (Code "apply-env") "具有两种不同的用途 (稍微算是滥用), "
      "这在之后的代码中会看得很清楚.")
   (CodeB "        ((lambda ,x* (assigned ,u* ,body))
         (define t*
         ...
         `(lambda ,y*
            ,(Let u* box* body^)))")
   (P (Code "convert") "的实质逻辑集中在对于"
      (Code "lambda") "和" (Code "let")
      "的处理上. 我们先看" (Code "lambda")
      ", " (Code "let") "是完全类似的. "
      "我们需要为被赋值的变量" (Code "u*")
      "生成相应的新变量" (Code "t*")
      ", 然后的处理基本上遵循之前的描述. "
      ...
      "的形状了, 它是被赋值变量" (Code "u")
      "和表达式" (Code "`(unbox ,u)")
      "的对应. 当然, 其实记录被赋值变量已经足够了, "
      "这里只是一种可能的实现方式.")
   (CodeB "        ((let ,bds (assigned ,u* ,body))
         (: bds
            (lambda (x* e*)
              (let* ((e* (map (convert env) e*))
                ...
                (Let y* e*
                     (Let u* box* body))))))")
   (P "对于" (Code "let") "的处理基本上和" (Code "lambda")
      "一样, 只是不要忘了转换右支" (Code "e*")
      ", 其上下文仍应为" (Code "env")
      ", 因为" (Code "let") "绑定管辖不到其右支.")
   (CodeB "        ((letrec ,bds ,body)
         (: bds
            (lambda (x* e*)
              (let ((e* (map (convert env) e*))
                    (body ((convert env) body)))
  ...
  ((convert '()) exp))")
   (P "收尾相当平淡无奇. 可以看到, 对于"
      (Code "letrec") "的处理完全是平凡的, "
      "因为该做的事情在" (Code "purify-letrec")
      "里就已经完成了.")
   (CodeB "&lt;exp> ::= (quote &lt;immediate>)
       |  &lt;uvar>
       |  (if &lt;exp> &lt;exp> &lt;exp>)
       ...
       |  (&lt;prim> &lt;exp>*)
       |  (&lt;exp> &lt;exp>*)")
   (P "最后我们给出" (Code "convert-assignments")
      "的输出语言的句法. 和" (Code "uncover-assigned")
      "之前的语言的句法相比, 赋值形式" (Code "set!")
      "消失了, 并且" (Code "letrec") "的右支变得更加狭隘, 只能是"
      (Code "lambda") "表达式了.")
   (P "以下是一些例子.")
   (CodeB "> (compil
   '(let ((x 0) (y 1) (z 2))
...
'(let ((t.3 (void)))
   ...
   (H2 "作业13: " (Code "optimize-direct-call"))
   (P (Code "optimize-direct-call")
      "是一个简单的优化步骤, 所以它不会改变句法. "
      "它的想法也非常简单, 就是将"
      (CodeB "((lambda (x ...) body) e ...)")
      "转换为"
      (CodeB "(let ((x e) ...) body)")
      "这类似于通过宏来实现" (Code "let") "的变换的逆.")
   (CodeB "(define (optimize-direct-call exp)
  (define (optimize exp)
    (match exp
      ((quote ,i) exp)
      (,x (guard (symbol? x)) x)
   ...
   (P "以上只能算是样板代码 (boilerplate code). "
      "若是借助于某种编译器框架, 或许可以写得更加简单.")
   (CodeB "      ((,rator . ,rands)
  ...
  (optimize exp))")
   (P "处理应用是" (Code "convert")
      "的实质部分, 其实也就是判断" (Code "rator")
      "是否是" (Code "lambda") "表达式而已.")
   (P "句法没有发生变化, 不再赘述. 以下是一些例子.")
   (CodeB "> (compil
   '((lambda (x y)
       (set! x (* x x))
       (set! y (* y y))
...
'((let ((x.0 '1)) (lambda (y.1) (+ x.0 y.1))) '2)")
   (H2 "作业13: " (Code "remove-anonymous-lambda"))
   (P "顾名思义, " (Code "remove-anonymous-lambda")
      "是为了消除匿名的" (Code "lambda")
      ". 所谓的匿名" (Code "lambda")
      ", 指的是没有直接出现在" (Code "let")
      "或者" (Code "letrec") "右支的" (Code "lambda")
      ". 对于藏在右支而非直接出现的情况, 当然也应该视为匿名的.")
   (P "对于匿名" (Code "lambda") "的处理是很简单的, "
      "即为其赋一个名字而已. 换言之, 匿名的"
      (CodeB "(lambda (x ...) body)")
      "将被转换为"
      (CodeB "(letrec ((t (lambda (x ...) body))) t)")
      "其中" (Code "t") "是一个新的变量. "
      "这样的转换不依赖于上下文, 直接进行即可.")
   (CodeB "(define (remove-anonymous-lambda exp)
  (define (remove exp)
    (match exp
      ((quote ,i) exp)
      (,x (guard (symbol? x)) x)
      ...
      ((lambda ,x* ,body)
       ...
       `(letrec ((,t ,l)) ,t))")
   (P "以上代码中只有对于" (Code "lambda")
      "的处理是实质性的. 能在这里被捕获的"
      (Code "lambda") "一定是匿名的, 否则会被"
      (Code "let") "和" (Code "letrec")
      "截胡. 我们放心地生成新的变量" (Code "t")
      ", 并施行前述的转换.")
   (CodeB "      ((let ,bds ,body)
       (: bds
          (lambda (x* e*)
       ...
       (: bds
          (lambda (x* e*)
            (let ((e* (map (lambda (e)
                  ...
                  (body (remove body)))
              (Letrec x* e* body)))))")
   (P "对于" (Code "let") "和" (Code "letrec")
      "的处理可以说是意料之中的事情, 它们大抵上是类似的. "
      (Code "let") "和" (Code "letrec")
      "都剔出了右支出现的" (Code "lambda") "单独处理. 对于"
      (Code "let") "而言, 这是为了避免引入冗余的"
      (Code "letrec") "绑定. 对于" (Code "letrec")
      "而言, 这则是必要的, 因为其右支只能是" (Code "lambda")
      ". 因此, 处理" (Code "letrec") "右支的" (Code "match")
      "仅有一个分支, 其他情况并不符合句法.")
   (CodeB "      ((,prim . ,rands)
       (guard (prim? prim))
      ...
      ((,rator . ,rands)
       (map remove exp))))
  (remove exp))")
   (P "收尾是平淡无奇的, 输出语言的句法也并没有发生变化. "
      "暂时我们还不能将" (Code "lambda")
      "从产生式中剔除, 因为有些" (Code "lambda")
      "是被" (Code "let") "绑定的. 不过, 经过接下来的"
      (Code "sanitize-binding-forms")
      "之后, 我们的确可以和" (Code "lambda") "说再见了.")
   (P "然而, 刚才我们的描述实际上并不够准确. 严格来说, "
      (Code "remove-anonymous-lambda")
      "的输出实际上是原本句法的一个子集. 在编写之后的"
      (Code "sanitize-binding-forms")
      "时, 既可以采用原本的句法, 也可以采用更加精细的句法. "
      "从理论上来说, 后者应该有一定的效率优势. "
      "当然, 说了这么多废话, 下面是更精细的句法.")
   (CodeB "&lt;exp> ::= (quote &lt;immediate>)
       |  &lt;uvar>
       |  (if &lt;exp> &lt;exp> &lt;exp>)
       |  (begin &lt;exp>+)
...
&lt;exp0> ::= &lt;lam> | &lt;exp>
&lt;lam> ::= (lambda (&lt;uvar>*) &lt;exp>)")
   (P "看起来有点fancy, 不是吗? 不过这能将" (Code "lambda")
      "表达式的出现限制在" (Code "let") "和"
      (Code "letrec") "的右支里.")
   (P "以下是一些例子.")
   (CodeB "> (compil
...
'(let ((c.0
        (let ((t.2 '0))
          (let ((x.1 (box t.2)))
            (letrec ((t.3
              ...
              t.3)))))
   (begin (c.0) (c.0) (c.0)))")
   (H2 "作业13: " (Code "sanitize-binding-forms"))
   (P (Code "sanitize-binding-forms")
      "的目的很简单, 也就是将" (Code "let") "绑定的" (Code "lambda")
      "表达式提取出来, 将其转换成由" (Code "letrec")
      "绑定的. 在这个pass之后, 一切" (Code "lambda")
      "表达式就都只能出现在" (Code "letrec") "的右支了, 而且当然"
      (Code "letrec") "的右支只能是" (Code "lambda") "表达式. "
      "另外, 虽然并非严格必要, " (Code "sanitize-binding-forms")
      "还会消除空" (Code "let") "和" (Code "letrec")
      ", 以及冗余的" (Code "begin") ". 在某种意义上, "
      "这个pass算是进行闭包变换之前的一块里程碑. "
      "因此, 修整一下程序也并非什么不可理解的事情.")
   (P "从概念上来说, 编写这个pass并不困难, 应该说一切的要素都似曾相识. "
      "不过, 最终的程序也不是很短, 这需要一点耐心.")
   (CodeB "(define (sanitize-binding-forms exp)
  (define (Let x* e* body)
    (if (null? x*)
        body
        `(let ,(map list x* e*) ,body)))
  ...
  (define (sanitize exp)
   ...
   (P "以上的代码中, 避免空绑定的" (Code "Let")
      "和" (Code "Letrec") ", 判断是否是"
      (Code "lambda") "表达式的谓词"
      (Code "lambda?") ", 都应该是容易理解的. "
      (Code "make-begin") "需要一点思考, "
      "因为它并不是在一般情况下消除冗余"
      (Code "begin") "的过程, 而是假定了"
      (Code "sanitize") "的确能够消除冗余的"
      (Code "begin") ". 这是结构递归的想法. "
      "至于一般的" (Code "make-begin")
      ", 可以写成"
      (CodeB "(define (make-begin exp*)
  (define (flatten exp*)
    (append-map
     (lambda (exp)
       (match exp
                    ...
                    (let ((x (car x*))
                      ...
                      (if (lambda? e)
                          ...
                          (iter x* e* (cons x xo*) (cons e eo*)
                                xl* el*)))))))))")
   (P "对于" (Code "let") "的处理是冗长的, 原因在于我们需要分类, "
      "这看起来有点像之前的" (Code "purify-letrec")
      "的某块内容, 但是概念上要简单得多. 我们只是要区分"
      (Code "lambda") "和非" (Code "lambda") ". "
      (S "最近在GitHub上逛逛别人写的P423作业, "
         "看到有人通过逐一判断的方式避免划分之后引入多余的"
         (Code "let") "或" (Code "letrec")
         ", 我觉得这种写法非常奇怪."))
   (CodeB "      ((letrec ,bds ,body)
       (: bds
          (lambda (x* e*)
  ...
  (sanitize exp))")
   ...
   (CodeB "&lt;exp> ::= (quote &lt;immediate>)
       ...
       |  (&lt;exp> &lt;exp>*)")
   (P (Code "sanitize-binding-forms")
      "的输出语言的句法如上, 虽然之前我们已经刻画过了. "
      "也就是说, 产生式里的" (Code "lambda")
      "表达式没有了. 现在" (Code "lambda")
      "表达式只能出现于" (Code "letrec")
      "的右支, 且" (Code "letrec") "的右支只能是"
      (Code "lambda") "表达式.")
   (P "以下是一些例子.")
   (CodeB "> (compil '(begin 1 (begin 2 (begin (begin 3 4) 5) 6) 7))
'(begin '1 '2 '3 '4 '5 '6 '7)")
   (CodeB "> (compil
   '(letrec ((even? (lambda (n)
                      (if (= n 0)
...
'((letrec ((h.0
    ...
    (letrec ((f.1
      ...
      (f.1 f.1)))
  '10)")
   (H2 "作业12: " (Code "uncover-free"))
   (P "实际上作业13还包括" (Code "optimize-known-call")
      ", 不过那是插入闭包变换中间的优化步骤, 所以先让我们来看作业12. "
      (Code "uncover-free") "是闭包变换的预备步骤, 其分析每个"
      (Code "lambda") "表达式的自由变量.")
   (CodeB "(define (uncover-free exp)
  (define (Lam lam k)
    (match lam
      ((lambda ,x* ,body)
       (uncover
         ...
         (lambda (exp u*)
           (uncover*
            ...
            (lambda (exp* v*)
              (k (cons exp exp*)
                 (U u* v*))))))))")
   (P "即便只是开头, 也能看出来" (Code "uncover-free")
      "和" (Code "uncover-assigned") "相当类似, 的确如此. "
      "当然, " (Code "uncover-assigned")
      "那里我们使用" (Code "values")
      "返回多值, 而这里我们使用额外的延续参数"
      (Code "k") ", 但这不是本质上的区别. "
      (Code "Lam") "专门用于处理" (Code "lambda")
      "表达式, 现在它只出现在右支. " (Code "Lam*")
      "处理一列" (Code "lambda") "表达式, "
      (Code "uncover*") "处理一列表达式, 它们和"
      (Code "uncover") "一道构成了互相递归的过程, "
      "各司其职地处理相应的句法.")
   (P "现在更详细地描述一下" (Code "Lam")
      ", 它利用" (Code "uncover")
      "分析" (Code "lambda") "的体的自由变量, "
      "然后除去被" (Code "lambda")
      "的形式参数绑定的那些变量, 就得到了整个"
      (Code "lambda") "表达式的自由变量. 最后, "
      "我们将自由变量记录于" (Code "free") "形式里.")
   (CodeB "  (define (uncover exp k)
    (match exp
      ((quote ,i) (k exp '()))
             ...
             e* (lambda (e* u*)
                  (uncover
                   ...
                   (lambda (body v*)
                     (k (Letrec x* e* body)
                        (D (U u* v*) x*)))))))))")
   (P "以上代码几乎只是常规而已. 不过, 或许应该再强调一下, "
      (Code "letrec") "的右支受到" (Code "letrec")
      "绑定的管辖, 而" (Code "let") "的右支不会受到"
      (Code "let") "绑定的管辖. 因此, 计算自由变量的代码也不太一样.")
   (CodeB "      ((,prim . ,rands)
       (guard (prim? prim))
       (uncover* rands
                 (lambda (rands u*)
  ...
  (uncover
   exp (lambda (exp u*)
         ...
         exp)))")
   (P "收尾的时候, 如果整个表达式还有自由变量, 那一定是不太对的. "
      "当然, 或许前面的pass已经能够排除这种情况了, "
      "但我们的确没有必要省略这样的检查, 这有点类似于防御式编程.")
   (CodeB "&lt;exp> ::= (quote &lt;immediate>)
       |  &lt;uvar>
       |  (if &lt;exp> &lt;exp> &lt;exp>)
       |  (begin &lt;exp>+)
...
'(let ((counter.0
   ...
   (H2 "作业12: " (Code "convert-closures"))
   (P "顾名思义, " (Code "convert-closures")
      "进行闭包变换, 不过实际上它只是闭包变换的第一步, 而或许"
      (Code "uncover-free") "应该算作闭包变换的第零步. "
      "对于这个变换, 我们最好先明确句法.")
   (CodeB "&lt;exp> ::= (quote &lt;immediate>)
       |  &lt;uvar>
       |  (if &lt;exp> &lt;exp> &lt;exp>)
       ...
       |  (&lt;prim> &lt;exp>*)
       |  (&lt;exp> &lt;exp> &lt;exp>*)")
   (P "只有" (Code "letrec") "和非原语应用的句法发生了变换, "
      "但是仅凭这句法的改变并不容易看出来这个变换究竟是在做什么. "
      "实际上, 这个变换主要是为接下来的一步"
      (Code "introduce-procedure-primitives")
      "作准备. 之后的" (Code "lambda")
      "将不包含有自由变量, 转而引入了一个新的参数, "
      "这也就是为什么新的句法之中" (Code "lambda")
      "至少会拥有一个参数, 这个参数出现在第一个位置上, "
      "代表所谓的" (Q "闭包指针 (closure pointer)")
      ". 什么是闭包指针呢? 顾名思义, 指向闭包的东西. "
      "那么闭包是什么呢? 这个应该没有Schemer不理解. "
      "简而言之, 技术性地, 一个闭包是一个向量, "
      "其由一个" (Q "函数指针 (标签)") "和诸自由变量的值构成, "
      "而所谓的函数指针里的函数, 其实就是这里不包含自由变量的新"
      (Code "lambda") ". " (Code "bind-free")
      "形式里的参数, 第一个位置上的也是闭包指针, "
      "之后的则是各个自由变量. 之后我们会看到, " (Code "lambda")
      "里原本的自由变量将会从闭包的分量之中提取出来. "
      (Code "closures") "形式只是为引入闭包作准备, "
      "这里每个闭包的数据分别是闭包的名字, 函数指针 (标签), 还有自由变量. "
      "我们之后将会引入构造闭包的原语. "
      "非原语应用的句法改变也是为了配合之后的变换, "
      "此时我们知道在精心安排之下, 出现在rator位置的表达式, "
      "其值应该是(作为向量的)闭包. "
      "这个闭包实际上不能直接应用, "
      "而之后的应用将会变为取出闭包的函数指针, "
      "将闭包和实际参数传递给函数指针. "
      "我们在这里所要做的事情只是复制rator罢了, "
      "并避免可能的重复求值.")
   (CodeB "(define (convert-closures exp)
  (define (convert exp)
    (match exp
      ((quote ,i) exp)
      (,x (guard (symbol? x)) x)
   ...
   (P "对于" (Code "letrec") "的处理是主要的复杂所在, "
      "但是我们所要做的只是生成所需的数据, 例如标签和闭包指针的名字, "
      "最后再按描述将它们放在一起.")
   (CodeB "      ((,prim . ,exp*)
       (guard (prim? prim))
...
'(letrec ((odd?.1$4
   ...
   (closures
    ...
    (even?.0 even?.0 '88)))")
   (H2 "作业13: " (Code "optimize-known-call"))
   (P "之前我们已经提及这个步骤, 但是按照逻辑顺序其应该放在这个位置.")
   (P "正如名字里所暗示的那样, "
      (Code "optimize-known-call")
      "是一个优化步骤, 所以应该不改变句法. "
      "不过, 实际上句法有一点微妙的改变.")
   (CodeB "&lt;exp> ::= (quote &lt;immediate>)
       |  &lt;uvar>
       ...
       |  (&lt;label> &lt;exp> &lt;exp>*)
       |  (&lt;exp> &lt;exp> &lt;exp>*)")
   (P "非原始应用现在也可能以标签开头, 这其实也正是"
      (Q "优化") "的用意. 接下来的"
      (Code "introduce-procedure-primitives")
      "实际上并不在乎这里是表达式还是标签, "
      "虽然实际上" (Code "convert-closures")
      ", " (Code "optimize-known-call")
      "和" (Code "introduce-procedure-primitives")
      "应该合并为一个步骤. 这可以免去诸多冗余的计算, "
      "但是这里分开的理由大概只是Kent Dybvig认为"
      "现在这种安排对于初学者更容易理解, "
      "可惜我并不这么觉得.")
   (P "本次优化的想法在于, 如果非原始应用的开头直接地引用一个闭包, "
      "我们知道 (或者读者还并不知道, 因为这是下一个变换所做的事情) "
      "接下来我们需要寻找其对应的函数指针, "
      "但是直接引用闭包意味着这个函数指针也可以直接引用, "
      "那么就无需间接的调用 (indirect call) 了. "
      "因此, 接下来的步骤里我们也要注意, "
      "如果非原始应用的开头已经是一个标签, "
      "那么就不能再动了.")
   (CodeB "(define (optimize-known-call exp)
  ...
  (define ((optimize env) exp)
       ...
       (: bds
          (lambda (x* e*)
            (let ((e* (map (optimize env) e*))
              ...
              (Let x* e* body)))))")
   (P "开头没什么好说的, 这里的" (Code "lookup")
      "是为了在环境" (Code "env")
      "之中查找" (Code "rator")
      "是否具有已知的对应标签. "
      "因此, 这个环境是由闭包的名字和其对应的标签的序对构成的. "
      "之后的步骤里, 我们要将非原始应用开头的闭包"
      "转换为获取这个闭包所对应的函数指针. "
      "而开头已经是标签的, 那就无需转换了.")
   (CodeB "      ((letrec ,bds (closures ,closures ,body))
       (define env^
         (append
          (map
           (lambda (c)
   ...
   (P "处理" (Code "letrec") "的部分很长, 但是主要的繁琐是扩展当前的环境"
      (Code "env") "为" (Code "env^") ". 这里需要小心的是, " (Code "letrec")
      "所绑定的这些标签的作用域也能覆盖到其右支的诸" (Code "lambda") "上.")
   (CodeB "      ((,prim . ,rands)
       (guard (prim? prim))
      ...
      ((,rator . ,rands)
       (let ((rator ((optimize env) rator))
         ...
         (if (symbol? rator)
             ...
             (cons rator rands))))))
  ((optimize '()) exp))")
   (P "对于非原始应用的处理是这次优化实质上的逻辑核心, 而其是由之前的"
      (Code "lookup") "所完成的. 我们试图找到作为符号的"
      (Code "rator") "所对应的标签, 但是找不到的话就维持原状.")
   (P "让我们来看一些例子.")
   (CodeB "> (compil
   '(letrec ((even? (lambda (n)
                      (if (= n 0)
...
'(letrec ((odd?.1$4
   ...
   (closures
    ((odd?.1 odd?.1$4 even?.0) (even?.0 even?.0$5 odd?.1))
    (even?.0$5 even?.0 '88)))")
   (P "这个例子是比较理想的情况, 因为所有的非原始应用的rator位置都被替换为了其所对应的标签.")
   (CodeB "> (compil
   '(((lambda (h)
        ((lambda (f) (f f))
         (lambda (g)
...
'(let ((t.17
   ...
   (t.17 t.17 '10))")
   (P "而又一个例子则是不怎么理想的情况.")
   (H2 "作业12: " (Code "introduce-procedure-primitives"))
   (P "顾名思义, " (Code "introduce-procedure-primitives")
      "引入了一些和过程 (或者说闭包) 相关的原语. "
      "这些原语包括" (Code "make-procedure") ", "
      (Code "procedure-set!") ", "
  ...
  (define (intro exp)
   ...
   (P "鉴于这次的代码很长, 所以每一块我也分割得很长, "
      "因为絮叨的叙述也无助于实际的理解. 对于"
      (Code "letrec") "的处理很长, "
      "但是实际上只是为了准备构造闭包而花了那么多篇幅. "
      "对于以标签开头的非原始应用需要单独处理, 所以引入了新的谓词"
      (Code "label?") ". 其他的非原始应用也仅是需要给"
      "rator位置加上" (Code "procedure-code") ".")
   (CodeB "  (define (lookup x cp free*)
      ...
      (match exp
        ...
        ((,rator . ,rands)
         (let ((rator ((Intro cp free*) rator))
           ...
           `((procedure-code ,rator) . ,rands))))))
  (intro exp))")
   (P (Code "lookup") "是为了将原本" (Code "lambda")
      "中的自由变量转换为对于闭包的分量的引用. "
      "实际上, 我们这里可以进行额外的检查, 确保"
      (Code "lambda") "里的变量要么来源于闭包, "
      "要么则是" (Code "lambda")
      "内部所绑定的 (包含其参数). "
      "不过, 我是有点懒散了, "
      "目前的情况是如果不在闭包所包含的自由变量之中, "
      "那么直接维持原样就好. "
      "另外我们需要注意的是, "
      "对于每个" (Code "lambda")
      "而言, 我们不需要像之前的环境那样维持一个"
      (Q "层次性") "的结构, 因为自由变量的值"
      "就保存在闭包里. 对于" (Code "lambda")
      "之中的" (Code "lambda")
      ", 它只是直接切换" (Q "上下文") ".")
   (P (Code "Intro") "和" (Code "intro")
      "几乎是一模一样的, 只是现在对于变量的处理需要查找. "
      "之前的不需要查找, 是因为我们知道它不在"
      (Code "lambda") "之中, 就谈不上自由变量的事情.")
   (P "以下是一些例子.")
   (CodeB "> (compil
   '(((lambda (h)
        ((lambda (f) (f f))
...
'(let ((t.17
   ...
   (P "有一点无聊但有趣的事情值得指出, 即Kent Dybvig有意将中间表示的句法"
      "安排得和Scheme类似. 所以说, 比如这里, 我们无需为此中间表示单独编写解释器"
      "来验证正确性, 其实只需要编写几个和闭包相关的过程即可."
      (CodeB "(define (make-procedure label length)
...
256")
   (P "最后让我们给出句法的变化."
      ...
      "实际上这里的" (Code "&lt;prim>")
      "也发生了改变, 多了几个过程. "
      "因此, 之后我们判断是否是原语需要使用另外不同的谓词.")
   (H2 "作业11: " (Code "lift-letrec"))
   (P (Code "lift-letrec") "的目的是为了将所有的" (Code "letrec")
      "都提升至顶层, 在一开始分配. 这次的pass也比较简单, "
      "因为所做的事情真的就只是提升" (Code "letrec")
      ". 而之所以可以这么做, 则有赖于之前各种的约简步骤, "
      "并不那么容易说清楚.")
   (CodeB "(define (lift-letrec exp)
  (define (Lam lam k)
    (match lam
      ((lambda ,x* ,body)
       (lift body
                       ...
                       (lambda (exp* binding*1)
                         (k (cons exp exp*)
                            (append binding*0
                                    binding*1))))))))")
   (P "老实说, 这些以" (Code "*")
      "为后缀的辅助过程实际上可以利用某种抽象来构造, "
      "但是我又没有什么动力这么做. "
      "当然了, 主要的逻辑还是在以下的"
      (Code "lift") "之中.")
   (CodeB "  (define (lift exp k)
    (match exp
      ((quote ,i) (k exp '()))
  ...
  (lift exp
   ...
   (P "而" (Code "lift")
      ...
      "绑定收集起来, 只留下body的部分. "
      "或许注意到" (Code "letrec")
      "是可能嵌套的, 但是当前的处理足够应付嵌套的情况. "
      "预期之中变换后的表达式是不含有" (Code "letrec")
      "的, 而这些" (Code "letrec")
      "的提升在很大程度上依赖于当前的"
      (Code "lambda") "已经没有了自由变量这一事实. "
      "若是还有自由变量, 则会导致作用域的错位. "
      "自由变量的值现在完全是藉由额外的闭包参数显式传递.")
   (P "现在让我们给出变换后的句法."
      (CodeB "&lt;program> ::= (letrec ((&lt;label> (lambda (&lt;uvar> &lt;uvar>*) &lt;exp>))*) &lt;exp>)
&lt;exp> ::= (quote &lt;immediate>)
       |  &lt;uvar>
       |  &lt;label>
       |  (if &lt;exp> &lt;exp> &lt;exp>)
       ...
       |  (&lt;prim> &lt;exp>*)
       |  (&lt;exp> &lt;exp> &lt;exp>*)")
      (Code "&lt;exp>") "的复杂度被进一步约简了, "
      "因为现在" (Code "letrec")
      "也没有了, 跑到了顶层, 并且"
      (Code "letrec") "也将" (Code "lambda")
      "带走了. 也就是说, " (Code "lambda")
      "在开头就被确定下来了.")
   (P "让我们来看一些例子.")
   (CodeB "> (compil
   '(((lambda (h)
    ...
    (let ((t.5 (void)) (t.4 (void)))
      (let ((odd?.1 (box t.5)) (even?.0 (box t.4)))
        (begin
          ...
          (let ((t.14 (unbox even?.0))) ((procedure-code t.14) t.14 '88))))))
#t")
   (H2 "作业11: " (Code "normalize-context"))
   (P "对于这个pass, 我们需要区分各种上下文, "
      "然后将程序塑造成合适的形状, 使得每个表达式都"
      (Q "各就其位") ". 这是迈向语言无关的中间表示的坚实一步. "
      "实际上, 存在着三类上下文:"
      (Ul (Li "值上下文: "
              "其期待着一个值, 比如说" (Code "let")
              "绑定的右支诸表达式.")
          (Li "谓词上下文: "
              "其期待着真假以确定控制流, 例如"
              (Code "if") "的第一个句法参数.")
          (Li "(副)作用上下文: "
              "结果无关紧要的地方, 例如"
              (Code "begin")
              "之中除了最后一个表达式之外的其他所有表达式."))
      "或许我们应该解释一下" (Code "normalize-context")
      "的目的. 比如说, 如果" (Code "(< 3 4)")
      "出现在值上下文之中, 那么这个值应该是什么呢? "
      "难道说是" (Code "#t") "吗? 问题在于, "
      "这并非语言无关的. 其他语言可能作出了不同的决策, "
      "例如在C语言里的话这个值是" (Code "1")
      ". 当然了, 不论采取哪一种特定的决策, "
      "这都不是语言无关的. 我们希望之后我们的中间表示"
      "能够与具体语言进行很好的分离. "
      "那么, 例如在这种情况下, "
      "我们需要显式添加对于谓词的返回值的具体描述.")
   (P "或许我们应该从给出句法开始, 实际上这次的句法比之前要复杂得多."
      (CodeB "&lt;program> ::= (letrec ((&lt;label> (lambda (&lt;uvar> &lt;uvar>*) &lt;value>))*) &lt;value>)
...
&lt;effect> ::= (nop)
          ...
          |  (&lt;effect-prim> &lt;value>*)
          |  (&lt;value> &lt;value> &lt;value>*)")
      "我这里所写下的句法比P423的作业描述要复杂. "
      "实际上, 这个句法是P423作业里的句法的一个子集. "
      "换言之, 作业里把句法放宽了, 减少了一些假设. "
      "之所以会这样, 一个直接的原因其实是我是倒着写的. "
      "因此, (或许)可以得到更多的假设. "
      "我决定暂时仍然保持这种更严格的句法, "
      "直到达到一个" (Q "新的阶段") ".")
   (P "这个句法几乎完全告诉了我们一切关于该怎么编写"
      (Code "normalize-context")
      "的线索. 实际上, 和之前一样, "
      "我们要做的事情只是遵循句法结构. "
      "对于三个非终结符" (Code "&lt;value>")
      ", " (Code "&lt;pred>") ", "
      ...
      "当然了, 实际上还有很多细节需要考虑, "
      "但这要我们结合具体的代码来说明.")
   (CodeB "(define (normalize-context program)
  ...
  (define (make-effect effect*)
    (let ((effect* (remove-all '(nop) effect*)))
      (cond ((null? effect*) '(nop))
            ...
            (else `(begin . ,effect*)))))")
   (P "这是一些预备, 但是现在还不是说明的好时机, "
      "要等待之后遇到了再说明. 不过, 顾名思义, "
      (Code "make-pred") "会构造一个"
      (Code "&lt;pred>") ", 而"
      (Code "make-effect") "会构造一个"
      (Code "&lt;effect>")
      ". " (Code "make-begin")
      "和" (Code "make-nopless-begin")
      "比较特殊, 它保持参数" (Code "e")
      "的上下文. 这从我们定义的方式中很容易看出来. "
      "然而, 这里和作业所提供的过程不太一样. "
      (CodeB "(define (make-nopless-begin x*)
  (let ([x* (remove '(nop) x*)])
    (if (null? x*)
        '(nop)
        (make-begin x*))))")
      "以上是作业里的代码, 其中" (Code "remove")
      "和Racket的不太一样, 去除的是所有值, "
      "我这里用的是自己定义的"
      (Code "remove-all")
      ". 然而, 这是无关紧要的差异, "
      "我想说的其他方面. "
      "一个显眼的差异是作业的版本只有一个参数, "
      "而我分成了两个. 这个差异想必读者容易理解, "
      "因为实际上" (Code "e*")
      "只可能是由" (Code "&lt;effect>")
      "构成的, 而" (Code "e")
      "则随情况而定, 三种可能都有. "
      ...
      ". 而且, 对于" (Code "e")
      "是" (Code "(nop)")
      "的情况, 作业的版本更像是歪打正着, 即去掉了最后的"
      (Code "(nop)") "也没事, 反正前面还有其他"
      (Code "&lt;effect>") "顶着. 即便都没有了, "
      "我们恰好就是需要" (Code "&lt;effect>")
      ", " (Code "(nop)") "还是对的. "
      "至少我不觉得这是正确的态度. "
      "我参考了许多别的人写的答案, "
      "它们在我使用" (Code "make-effect")
      "的地方也使用了" (Code "make-nopless-begin")
      ", 这两个过程的意蕴是不同的. "
      (Code "make-effect") "只是为了根据数个"
      (Code "&lt;effect>") "来复合成一个"
      (Code "&lt;effect>")
      ". " (Code "make-nopless-begin")
      "则是将数个" (Code "&lt;effect>")
      "和一个具有特定上下文的表达式合成, "
      "并保持后者的上下文. "
      "我觉得我这种做法是更加清晰明确的. ("
   ...
   (P "虽然这里每一行都是逻辑点, "
      ...
      "而原始应用中最值得注意的大概是"
      "意图利用副作用的原语的显式返回值是"
      (Code "(void)") ". 当然了, " (Code "void")
      "的确也是值原语.")
   (CodeB "  (define (Pred exp)
    (match exp
      ((quote ,i)
   ...
   (P (Code "Pred") "大体上和"
      ...
      "即可. 当然了, 就此还有另外的优化空间, "
      "和常量折叠与传播有关. "
      "然而, 我觉得这些也可以放在另外的pass里, "
      "完全不在这里做了.")
   (CodeB "  (define (Effect exp)
    (match exp
      ((quote ,i) '(nop))
      ...
      ((,rator . ,rands)
       (map Value exp))))")
   (P (Code "Effect") "最有趣的地方大概是对于原始应用中的"
      "值应用和谓词应用, 我们可以直接丢弃结果, "
      "只是利用参数的副作用. 而且, 有的参数毫无副作用, "
      "所以可以直接丢弃. 这的确相当有趣, "
      "然而其实我知道这里我写的东西是成问题的, "
      "而作业对此的描述也是有问题的. "
      "我感觉Kent Dybvig应该知道一点, "
      "但是他只是想让作业简单一些. "
      "然而, 我感觉从我参考的几个人的答案来看, "
      "他们似乎都没有意识到这个问题, "
      "甚至有个人忘了作业里描述的这一茬了, "
      "直接把参数全扔了. 实际上, 这里的问题在于, "
      "值原语和谓词原语在应用的时候也可能抛出异常, "
      "这在实际编译器进行优化的时候是必须考虑进去的事情. "
      "当然了, 作业做了一点简化, 进行了善意推定, "
      "假设程序都是对的, 这种应用不会抛出异常. "
      "我没有选择打破作业要求, 但是我认为这是值得注意的. "
      "(上一段我提及的优化其实和这里的如出一辙就是了, "
      "不过那里我已经考虑了这个问题.)")
   (CodeB "  (match program
    ((letrec ,bds ,body)
     (: bds
   ...
   (P "最后的收尾完全在意料之中. "
      "不过, 或许我们应该注意到, 虽然"
      (Code "lambda") "的体是在值上下文中的, "
      "但是非原始应用也可以出现在副作用的上下文中, "
      "这已经体现在前面" (Code "Effect") "的代码之中了.")
   (P "现在让我们来看一些例子.")
   (CodeB "> (normalize-context
   '(letrec ()
...
'(letrec ((odd?.1$4
   ...
   (let ((odd?.1 (make-procedure odd?.1$4 '1))
     ...
     (begin
       ...
       (even?.0$5 even?.0 '88))))")
   (P "写得" (Q "正常") "的程序, " (Code "normalize-context")
      "就没什么事情可做.")
   (CodeB "> (compil
   '(lambda (x)
      (&lt; x 3) x))
'(letrec ((t.1$2 (lambda (cp.3 x.0) x.0)))
   (let ((t.1 (make-procedure t.1$2 '0))) t.1))")
   (P "这个转换严格说来的确就不太对, 直接抛弃了潜在可能出现异常的"
      (Code "(&lt; x 3)") ".")
   (P "或许最后提点无聊的事情, 作业里说" (Code "(void)")
      "在副作用上下文中会变成" (Code "(nop)")
      ", 这实际上不需要特殊处理, 因为这是将" (Code "void")
      "视为值原语的自然结果.")
   (H2 "作业10: " (Code "specify-representation"))
   (P (Code "specify-representation")
      "的目的在于选取一种对于值 (也称对象) 的特定的表示 (或者说编码) 方式, "
      "然后将原语转换为低级的内存操作. 鉴于Scheme是一个动态定型的编程语言, "
      "这种编码需要在运行时带上标记 (tag). 标记的选取通常来说具有一定的任意性, "
      "但是也存在着一些巧思可以优化性能. 在概念上, 值(对象)是一个(带标记的)引用. "
      "不过, 所谓的立即值 (immediate) 可以直接在这个引用里被编码. "
      "然而, 这应该更多地被视为实现相关的想法.")
   (P "内存操作是以下三种:"
      (Ul (Li (Code "(alloc " $n ")")
              ": 分配" $n "个字节, 然后返回起始的地址.")
          (Li (Code "(mref " $n " " 'offset ")")
              ": 获取内存位置在" (&+ $n 'offset)
              "处起的值 (从概念上来说, 是一个64位的数码).")
          (Li (Code "(mset! " $n " " 'offset " " 'val ")")
              ": 设置内存位置自" (&+ $n 'offset)
              "处起的值为" 'val "."))
      "我们对于我们的64位编译器所采取的编码方法是用64位来表示一个指针, "
      "而这个指针的低3位是标记. 换言之, 在抹去标记之后, 真实的指针按8字节 (64位) 对齐了. "
      "另一种看待方式是每个真实地址都是8的倍数, "
      "所以其间的数字就可以用来表示对象的类型了, 即从真实地址加上0到加上7.")
   (P "定长数 (fixnum) 的标记是" (Code "000") ". 换言之, 我们有61个位来表示整数值, "
      "然后这个值会被左移3位, 或者说乘上8. "
      "选" (Code "000") "作为标记对于fixnum这个类型的运算会比较便利. "
      (Code "+") "和" (Code "-") "不需要额外调整, "
      (Code "*") "需要适当地给其中一个参数施行算术右移3位. "
      "选取" (Code "000") "作为标记还有一个好处, "
      "那么就是对于向量之类的需要按位置索引的数据结构, "
      "原本的数字就乘上了8, 恰好相当于64位, 所以也无需额外的调整.")
   (P (CodeB "(define $false #b00000110)
...
(define tag:vector    #b111)")
      "剩下的一些标记, 从这部分代码中就能自行看出来. "
      "我们将" (Code "110") "拿出来用于表示几个常量.")
   (P "将原语转换为内存操作其实没有什么技巧, 只是要确实地想明白到底做了什么. "
      "比如说, 对于一个序对, 我们需要分配16个字节, 其实就是两个64位, "
      "一个是" (Code "car") "部分, 一个是" (Code "cdr") "部分. "
      "而在对于序对的表示里, 我们只需要指向" (Code "car") "部分即可, "
      (Code "cdr") "部分可以根据" (Code "car") "的实际地址加上8得到, 因为是连续分配的. "
      "为了取得一个序对的" (Code "car") "部分, 只需要给序对的编码减去序对的标记就可以得到实际地址. "
      (Code "cdr") "部分如前所述就是再加上8而已. "
      (Code "set-car!") "和" (Code "set-cdr!") "也是类似的, 只是被转换为了"
      (Code "mset!") "而非" (Code "mref") ". 其他的数据类型的操作基本上也遵循着类似的模式, "
      "只不过可能需要分配的内存空间是不定的, 需要进行计算. 索引的位置也需要根据实际情况进行计算.")
   (P "对于数据类型谓词的处理有另外的套路, 其实就是通过"
      (Code "logand") "和某个掩码 (mask) 来得到其标记. "
      (Code "boolean?") "这个谓词有点特殊, 需要ad hoc的处理方式."
      (CodeB "(define mask:boolean   #b11110111)
(define tag:boolean    #b00000110)")
      "其他的掩码很明显就是(二进制的)" (Code "111")
      ", 配合之前已经定义了的标记使用. "
      "对于算术谓词无需额外处理, 而" (Code "eq?")
      "其实只是指针比较, 所以转换为" (Code "=") "即可.")
   (CodeB "  (define Value-prim
    (case-lambda
      ((void) $void)
      ((uop x)
       (case uop
  ...
  (define Effect-prim
   ...
   (P "对于原语的处理是这次pass的核心, 也就是以上所呈现的内容了. "
      "其实里面有的转换没有那么符合Scheme的语义, 比如说对于" (Code "box")
      "的处理, 其实我们应该先对于参数求值, 然后再分配, "
      "而不是像现在这样先分配, 再对参数求值. "
      "不过, 对于当前的Scheme子集而言, 其实两者没什么区别. "
      "也有其他的地方需要特别注意, 比如说对于" (Code "make-vector")
      "的处理, 其参数在为数字或符号的时候有较简单的处理方式, "
      "而在其他一般情况下我们一定要添加" (Code "let")
      "绑定来避免对于其参数进行重复求值, 其中一个直接的原因在于参数可能包含副作用. "
      "(实际上, 我参考的三份作业答案里, 只有王垠意识到了这件事情, 其他两个人这里写的都是错的.) "
      "老实说, 这里对于算术的处理有点粗糙, 没有考虑溢出之类的情况, 所以并不能保证在任何时候都正确, "
      "但是我们暂时就满足于这样的处理了, 之后可能会加以改进.")
   (CodeB "(define (specify-representation program)
  (define Value-prim
    (case-lambda
      ((void) $void)
      ((uop x)
     ...
     (: bds
        (lambda (x* e*)
          (let ((e* (map (lambda (e)
                ...
                (body (Value body)))
            (Letrec x* e* body)))))))")
   (P "以上则是完整的代码, 不过代码的其他地方从某种意义上说都挺无聊的.")
   (P "现在让我们给出输出语言的句法."
      (CodeB "&lt;program> ::= (letrec ((&lt;label> (lambda (&lt;uvar> &lt;uvar>*) &lt;value>))*) &lt;value>)
&lt;value> ::= &lt;triv>
         |  (if &lt;pred> &lt;value> &lt;value>)
...
&lt;binop> ::= + | - | * | sra | logand
&lt;relop> ::= = | &lt; | > | &lt;= | >=")
      "可以看到, 原语都已经消失了, 取而代之的是内存操作和"
      (Code "&lt;binop>") "以及" (Code "&lt;relop>") ".")
   (P "让我们再来看一些例子. 正如之前所言, Kent Dybvig故意将句法安排得近于Scheme, "
      "以使得在某种程度上验证可以变得稍微容易一点 (其实不符合" (Q "do the right thing")
      "的精神). 为此, 我们需要补充几个过程, 不过这些过程也写得很粗糙, 只适用于排除"
      (Q "非常明显") "的错误. (不过, 实际上这些过程在我编写pass时的确很有用.)"
      (CodeB "(define (true) #t)
(define (false) #f)
(define (nop) (void))
...
(define (get-result encoding)
  (case (logand encoding #b111)
    ...
    (else (error 'get-result &quot;unknown encoding ~s&quot; encoding))))")
      "其中" (Code "get-result") "是用来将编码转换为可读的Scheme值的.")
   (P "好了, 让我们开始呈现实际的例子.")
   (CodeB "> (compil
...
'(3 . 8)")
   (P "这是第四个例子.")
   (H2 "作业9: " (Code "uncover-locals"))
   (P (Code "uncover-locals")
      "是比较简单的, 因为它只是搜集被" (Code "let")
      "所绑定的变量. 存在着两种直接的实现策略, "
      "一种是返回多值, 另一种是使用副作用. "
      "我使用了前者, 而没有实现后者, "
      "不过显然后者代码应该要简单很多.")
   (P "这个pass的输出句法为"
      (CodeB "&lt;program> ::= (letrec ((&lt;label> (lambda (&lt;uvar> &lt;uvar>*) &lt;body>))*) &lt;body>)
&lt;value> ::= &lt;triv>
         |  (if &lt;pred> &lt;value> &lt;value>)
         |  (begin &lt;effect>* &lt;value>)
         |  (let ((&lt;uvar> &lt;value>)*) &lt;value>)
...
&lt;binop> ::= + | - | * | sra | logand
&lt;relop> ::= = | &lt; | > | &lt;= | >=")
      "可以看到, 仅仅是多了" (Code "locals")
      "形式用于放置搜集到的变量. 实际上, "
      "其余部分不应该发生任何变化.")
   (CodeB "(define (uncover-locals program)
     ...
     (: bds
        (lambda (x* e*)
          (let ((e* (map (lambda (e)
                ...
                (body (make-body body)))
            (Letrec x* e* body)))))))")
   (P "因为这次的代码没有任何难度, 我就直接放在这里了. "
      "注意到我们使用了" (Code "append") "而非"
      (Code "U") "来合并变量集, 鉴于这些变量实际上只可能在"
      (Code "let") "的左支里出现一次.")
   (P "这次也没有例子, 因为我们感觉例子并不需要.")
   (H2 "作业9: " (Code "remove-let"))
   (P "顾名思义, " (Code "remove-let")
      "将所有的" (Code "let") "都去除了, "
      "转换为了等价的" (Code "set!")
      "形式 这个pass在很大程度上依赖于"
      (Code "parse-scheme") "所施行的" $alpha
      "变换, 不过就此方面其他绝大多数pass也是一样的.")
   (P "鉴于我们之前已经在赋值变换中去除了"
      (Code "set!") "形式, 所以说在"
      (Code "remove-let") "之后, "
      "对于每个变量也只会有单次赋值.")
...
&lt;relop> ::= = | &lt; | > | &lt;= | >=")
      "可以看到, " (Code "let") "消失了, 而"
      (Code "&lt;effect>") "的产生式里多了一个"
      (Code "set!") "形式.")
   (CodeB "(define (remove-let program)
  (define (If Context pred e1 e2)
     ...
     (: bds
        (lambda (x* e*)
          (let ((e* (map (lambda (e)
                ...
                (body (Body body)))
            (Letrec x* e* body)))))))")
   (P "这同样算不上困难, 所以说也没有必要添加什么注解.")
   (P "让我们来看一些例子, 请注意我们将"
      (Code "locals") "视为某种形式的绑定, 即"
      (CodeB "(define-syntax locals
  (syntax-rules ()
    ((_ (x ...) exp)
     (let ((x (void)) ...) exp))))"))
   (CodeB "> (compil
...
> (get-result 73)
'(3 . 8)")
   (H2 "作业8: " (Code "verify-uil"))
   (P "顾名思义, " (Code "verify-uil") "是一个验证pass. "
      "然而, 我们所使用的句法从形式上说和前一个的输出不同. "
      "实际上, 这里的输入句法是"
      (CodeB "&lt;program> ::= (letrec ((&lt;label> (lambda (&lt;uvar> &lt;uvar>*) &lt;body>))*) &lt;body>)
&lt;value> ::= &lt;triv>
         |  (if &lt;pred> &lt;value> &lt;value>)
...
&lt;relop> ::= = | &lt; | > | &lt;= | >=")
      ...
      "其实就是出现在尾位置的"
      (Code "&lt;value>")
      ". 不过, 另外两种表达式本来就不可能出现在尾位置上. "
      "从某种意义上说, 这暗示了在之后的pass里, "
      (Code "&lt;value>") "将要被消去. 至于"
      (Code "verify-uil") "本身, 我认为没有什么好说的.")
   (CodeB "(define (verify-uil program)
  (define (triv? triv)
    (or (int64? triv)
        (uvar? triv)
        (label? triv)))
     ...
     (: bds
        (lambda (x* e*)
          (let ((e* (map (lambda (e)
                ...
                (body (Body body)))
            (Letrec x* e* body)))))))")
   (P "当然, 还要另外一种写法, 就是写成谓词的形式, "
      "因为这只是一个验证步骤, 输入和输出应该是一模一样的.")
   (P "不过, 实际上我们之后将会采取下列句法, "
      "其只是对于之前的句法的小修饰, 旧句法所指称的AST集合"
      "是新句法所指称的AST集合的一个子集."
      (CodeB "&lt;program> ::= (letrec ((&lt;label> (lambda (&lt;uvar>*) &lt;body>))*) &lt;body>)
&lt;value> ::= &lt;triv>
         |  (if &lt;pred> &lt;value> &lt;value>)
         |  (begin &lt;effect>* &lt;value>)
         |  (alloc &lt;value>)
...
&lt;binop> ::= + | - | * | sra | logand
&lt;relop> ::= = | &lt; | > | &lt;= | >="))
   (H2 "作业6: " (Code "remove-complex-opera*"))
   (P "顾名思义, " (Code "remove-complex-opera*")
      "消除了原始应用和非原始应用中的复杂表达式. "
      "何谓复杂? 只要不是" (Code "&lt;triv>")
      ", 那都是复杂的.")
   (P "那么怎么消除应用中的复杂表达式呢? "
      "其实就是通过" (Code "set!")
      "引入新的绑定, 这让人想到所谓的ANF变换.")
   (P "我们还是使用CPS来返回多值, 不过现在有两种不同的延续. "
      "对于目的是得到" (Code "&lt;triv>")
      "的, 我们除了平凡表达式, 还需要返回"
      (Code "set!") "绑定序列和引入的新变量 "
      "(后者" (Code "locals") "形式需要的信息). "
      "对于目的是得到值表达式, 谓词表达式, 副作用表达式以及尾表达式的, "
      "我们返回表达式和新引入的变量即可, 无需绑定序列.")
   (CodeB "(define (remove-complex-opera* program)
     ...
     (: bds
        (lambda (x* e*)
          (let ((e* (map (lambda (e)
                ...
                (body (Body body)))
            (Letrec x* e* body)))))))")
   (P "从整体上来说, 这次的代码仍然是对应于各种非终结符的过程的交织. "
      "从细节上来说, 我们需要小心地合并" (Code "set!")
      "绑定, 避免出现求值顺序的错误. 当然了, 和ANF以及CPS变换一样, "
      (Code "remove-complex-opera*") "强加了一种求值的顺序, "
      "或者说使得求值顺序显式化.")
   (P "以下是一些例子.")
   (CodeB "> (compil
   '(let ((x 0) (y 1) (z 2))
...
&lt;relop> ::= = | &lt; | > | &lt;= | >=")
      ...
      "的生存空间被极大压缩了, 只活在"
      (Code "set!") "的右支里.")
   (H2 "作业6: " (Code "flatten-set!"))
   (P (Code "flatten-set!")
      "将清理" (Code "set!") "的右支, "
      "以扫除" (Code "&lt;value>") ".")
   (CodeB "  (define ((simplize-set! x) value)
    (match value
      ((if ,pred ,v1 ,v2)
   ...
   (P "从这部分代码中可以看到, 清理的策略也很简单, "
      "只需要递归地处理" (Code "if") "和"
      (Code "begin") ".")
   (CodeB "(define (flatten-set! program)
  (define (If Context pred e1 e2)
    `(if ,(Pred pred) ,(Context e1) ,(Context e2)))
  (define (Begin Context exp+)
    (:begin exp+
...
> (get-result 361)
'(1 4 9 16 25 36 49 64 81 100 121 144 169 196 225 256 289 324 361 400)")
   (H2 "作业6: " (Code "impose-calling-conventions"))
   (P "从这个pass开始, 事情开始起变化. "
      "这个pass和之后的pass, 许多都牵涉很多次作业, "
      "我选择尽量将pass的作业编号标示为其首次出现的地方.")
   
   (H2 "作业8?: " (Code "expose-allocation-pointer"))
   (H2 "杂记")
   (H3 "活跃分析 (Liveness Analysis)")
   (P "A variable or register is in use, or live, at any given point, "
      "if the variable’s value might yet be needed by the program. "
      "In general, this is an undecidable property, "
      "so we conservatively assume that the variable is live "
      "if we cannot prove that it is not live. "
      "The traditional conservative approximation, which we use, "
      "is that a variable is live at a given point "
      "if any reference to the variable occurs along any flow of control "
      "from the given point before the variable is killed (overwritten) "
      "by an intervening assignment to the variable.")
   (P "一个变量或者寄存器在某个给定点在使用中或者说活着, "
      "如果变量(或者寄存器)的值还可能(之后)要被程序使用. "
      "在一般情况下, 这是一个不可判定性质, "
      "所以说我们保守地认为变量(或者寄存器)是活着的, "
      "如果我们无法证明它不是活着的. "
      "我们所使用的传统保守近似是, 一个变量(或者寄存器)在给定点活着, "
      "如果从给定点起在这个变量被某个介入的赋值杀死 (覆写) 之前"
      "沿着任意的控制流出现了对于这个变量(或者寄存器)的引用.")
   ))