...
'(letrec ((t.1$2 (lambda (cp.3 x.0) x.0)))
   ...
   (P (Code "specify-representation") "的目的是为了选取特定的表示方法, "
      "或者说编码方法, 然后将许多原语转换为更低层次的内存操作. "
      "既然Scheme是一个动态定型的语言, 所以说我们对于值 (也称对象) 的表示"
      "在概念上是一个带标记的引用. 然而, 所谓的立即值可以直接在引用之中编码, "
      "无需指向内存中的位置. 但是, 这更多是实现上的而非概念上的. "
      "标记的选取通常来说具有一定的任意性, 然而其中也有一些巧思可以优化性能.")
   (P "我们对于我们的64位编译器所采取的编码方法是用64位来表示一个指针, "
      "而这个指针的低3位是标记. 换言之, 在抹去标记之后, "
      "真实指针按8字节 (64位) 对齐了. 另一种看待方式是"
      "每个真实地址都是8的倍数, 所以其间的数字就可以用来表示对象的类型了, "
      "即从真实地址加上零到加上七.")
       ...
       (let ((map.0
         ...
         (begin
           ...
           (map.0$6
            ...
            t.4))))))
'(1 4 9)")
   (CodeB "> (compil ''#(1 #t #f () (2 3) #(4 5)))
'(letrec ()
   (let ((t.0
          (let ((vec.1
                 (let ((vec.3 (+ (alloc 56) 7)))
                   (begin (mset! vec.3 -7 48) vec.3))))
            (begin
              (mset! vec.1 1 8)
              (mset! vec.1 9 14)
              (mset! vec.1 17 6)
              ...
              (mset!
               ...
               (let ((car.7 16)
                     (cdr.8
                      (let ((car.4 24) (cdr.5 22))
                        (let ((pair.6 (+ (alloc 16) 1)))
                          (begin
                            (mset! pair.6 -1 car.4)
                            (mset! pair.6 7 cdr.5)
                            pair.6)))))
                 (let ((pair.9 (+ (alloc 16) 1)))
                   (begin
                     (mset! pair.9 -1 car.7)
                     (mset! pair.9 7 cdr.8)
                     pair.9))))
              (mset!
               ...
               41
               (let ((vec.2
                      (let ((vec.10 (+ (alloc 24) 7)))
                        (begin (mset! vec.10 -7 16) vec.10))))
                 (begin (mset! vec.2 1 32) (mset! vec.2 9 40) vec.2)))
              vec.1))))
     t.0))")
   (CodeB "> (get-result
   (letrec ()
