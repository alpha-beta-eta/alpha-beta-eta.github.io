...
(define control0.html
  (TnTmPrelude
   ...
   (P "指称语义由Christopher Strachey, Dana Scott, "
      "Christopher Wadsworth等人于20世纪60年代末提出, "
      "是一种以数学精确性定义程序意义的方法. "
      "指称语义以组合性的方式, 为编程语言的每个句法元素 "
      "(表达式, 语句, 函数等) 关联一个数学对象.")
   (P "例如, 一个包含变量的整数算术表达式" $e
      "的意义, 可以定义为从存储到整数的函数, "
      "其中存储将整数关联到变量:"
      (MB (&: (deno $e) (&-> $Store $Int)))
      "其中"
      (MB (&= $Store (&->^fin $Variable $Int)))
      "这个指称函数由" $e
      "的结构上的归纳和情形分析定义, 例如:"
      (MB (set-attr*
           (&Table
            ...
            ((ap (deno $x) $sigma)
             $= (app $sigma $x)
             (: "如果" $x "是一个变量"))
            ((ap (deno (&+ $e_1 $e_2)) $sigma)
             $= (&mod (@+ (ap (deno $e_1) $sigma)
                          (ap (deno $e_2) $sigma))
                      $2^32))
            ((ap (deno (&* $e_1 $e_2)) $sigma)
             $= (&mod (@c* (ap (deno $e_1) $sigma)
                           (ap (deno $e_2) $sigma))
                      $2^32)))
           'columnalign
      ...
      "于是所有的算术运算都用要模"
      $2^32 ".")
   (P "语句" $s "的指称语义, 作为第一次近似, 是一个"
      (Em "存储转换器 (store transformer)")
      ": 一个函数将" $s "执行之前的存储映射至"
      $s "执行之后的存储."
      (MB (&: (deno $s) (&-> $Store $Store)))
      "例如, 考虑赋值" (&:= $x (&+ $x $1))
      ". 从存储" $sigma "开始, 其终于存储"
      (&= $sigma^ (extend $sigma
                          $x (&mod (@+ (app $sigma $x) $1)
