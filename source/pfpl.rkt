...
(define pfpl.html
  (parameterize ([parameter:format-num format-num])
    (TnTmPrelude
     ...
     (H2. "古典逻辑")
     (P "在构造性逻辑里一个命题为真恰当其有一个证明 "
        "(即一个根据公理和假设的对其的推导), "
        "而为假恰当其有一个反驳 (即一个根据其为真的假设对于矛盾的推导). "
        "构造性逻辑是一种正面证据的逻辑. "
     ...
     (H2. "")
     (H2. "嵌套并行")
     (P "并行计算通过允许多个计算同时施行而寻求减少程序运行时间的机会. "
        "例如, 如果我们希望将两个数相加, "
        "并且每个数字都是由一个复杂的计算给出的, "
        "那么我们或许会考虑对于这些被加起来的数字同时求值, "
        "然后计算它们的和. "
        "利用并行性的能力受到程序各部分之间的依赖关系的限制. "
        "显然, 如果一个计算依赖于另一个计算的结果, "
        "那么我们就别无他法, 只能选择将其顺序执行, "
        "如此才能将第一个计算的结果传播到第二个计算. "
        "由此而言, 子计算之间的依赖越少, 并行性的机会也就越多.")
     (P "本章我们讨论语言PPCF, 其是以嵌套并行对于PCF的扩展. "
        "嵌套并行具有层次结构, 其来源于fork两个或更多的并行计算, "
        "然后在继续处理之前join这些计算以合并它们的结果. "
        "嵌套并行也称fork-join并行. "
        "我们将会考虑嵌套并行的两种形式的动态语义. "
        "第一种是结构动态语义, "
        "复合表达式上的单一转换可能牵涉其分量表达式上的多重转换. "
        "第二种是成本动态语义 (第7章引入的), "
        "其专注于并行程序的顺序和并行复杂度 "
        "(也被称为work和depth, 后者也可以称为span) "
        ", 通过联系每个计算以一个串并图 "
        "(series-parallel graph).")
     (H3. "二元fork-join")
     (P "PPCF的句法以如下构造扩展了PCF:"
        (MB (&Table
             ($Exp $e $::= (Appl $par $e_1 $e_2 (Bind $x_1 $x_2 $e))
                   (App $par (&= $x_1 $e_1) $and
                        (&= $x_2 $e_2) $in $e)
                   "并行let")))
        "变量" $x_1 "和" $x_2 "只在" $e "之中被绑定, "
        "而不在" $e_1 "或" $e_2 "之中被绑定, "
        "这保证了它们不能互相依赖, 因而可以同时求值. "
        "变量绑定表示了两个并行计算" $e_1 "和" $e_2
        "的fork, 而body " $e "则表示了其join.")
     (P "PPCF的静态语义以如下用于并行let的规则丰富了PCF的静态语义:"
        (MBL "(37.1)"
             (&rule
              (G!- (&: $e_1 $tau_1))
              (G!- (&: $e_2 $tau_2))
             ...
             (&rule
              ...
              (&\|->_par
               ...
               (Subst (&cm $e_1 $e_2)
                      (&cm $x_1 $x_2)
                      $e))))
        "并行动态语义抽象掉了处理容量上的任何限制; "
        "这样的限制于第37.4节进行考虑.")
     (P "隐式并行定理陈述了顺序和并行动态语义恰好相合的事实. "
        "由此我们无需关心并行程序的语义 "
        "(其意义由顺序动态语义给出), "
        "而只需要关心程序的效率. "
        "从实际角度而言, 这意味着程序可以在顺序平台上进行开发, "
        "即便程序的意图是在并行平台上运行, "
        "因为程序的行为并没有受到我们是用顺序"
        "还是并行动态语义执行程序的影响. "
        "鉴于顺序动态语义是确定性的 "
        "(每个表达式至多只有一个值), "
        "隐式并行定理可以推出并行动态语义也是确定性的. "
        "出于这个原因, 隐式并行定理也可称为确定性并行定理. "
        "这个术语强调了确定性的并行 (本章的主题) "
        "和非确定性的并发 (第39和40章的主题) 之间的区别.")
     (P "隐式并行定理的证明"
        )
     (H2. "未来和投机")
     (H2. "进程演算")
