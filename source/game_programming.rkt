...
you exit the whole game.")
   (H3. "渲染")
   (P "到目前为止我们的游戏只是以句子作为反馈. "
      "更一般地, 我们需要系统呈现当前世界状态的方式, "
      "即渲染. 虽然渲染在图形学中有特别的含义, "
      "但是这里我们将一切呈现世界状态的过程称为渲染, "
      "不论其是否涉及图形.")
   (H3. "游戏AI")
   (H4. "对话树和行为树")
   (P "对话树和行为树都是很古早的想法了, "
      "然而仍然普遍运用于各种各样的游戏之中. "
      "使用对话树的最典型例子是所谓的视觉小说, "
      "不过实际上对话树可能并不是一个树, 而是一个图, "
      "甚至我们所说的对话树还可能涉及一些编程语言的机制, "
      "典型的情况是设置一些FLAG或者说变量用于条件判断. "
      "行为树在某种意义上和对话树是一模一样的, "
      "只是不限于所谓的对话而已.")
   
   (H4. "寻路")
   (P "寻路的基本模板是所谓的A*算法, 其可以视为一致代价搜索的推广. "
      )
   (H4. "Monte-Carlo树搜索")
   (P "Monte-Carlo树搜索的运用舒适区大概是一些离散决策AI, "
      "尤其是棋类游戏或者类似物. "
      )
   (H3. "程序化生成")
   (P "虽然程序化生成在某种意义上也应该算作AI, 但是我觉得值得单独成为一节. "
      "虽然手工设计和制作的素材和资源也相当重要, "
      "程序化生成在很多时候仍然是不可或缺的.")
   (P "第一眼看到程序化生成的时候, "
      "人们可能会想到程序化生成的游戏地图 (或者也可以说是世界?), "
      "例如Persona 3所做的那样 (虽然这个游戏的程序化生成地图饱受诟病). "
      "不过, 当然还有很多地方需要用到一些程序化生成技术, "
      "例如许多游戏的某些视觉效果 (水的波纹之类的).")
   (H3. "DSL (领域特定语言)")
   (P "游戏制作是控制复杂度的艺术, 而控制复杂度的终极方式是元语言抽象. "
      "通过DSL来描述游戏逻辑是游戏开发领域的一种惯例, "
      "尽管许多游戏开发人员并没有严肃的编程语言理论背景. "
      "如果举一个实际的例子的话, 我首先会想到Naughty Dog工作室"
      "所使用的一种嵌入Racket的DSL, 其用于神秘海域和最后生还者等游戏的开发. "
