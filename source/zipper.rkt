...
(define zipper.html
  (TnTmPrelude
   ...
   #:css "styles.css"
   (H1. "The Zipper (函数式珍珠)")
   (P "本文的作者Gérard Huet以简单明快的笔触优雅地呈现了zipper的想法.")
   (H2 "Capsule Review")
   (P "几乎每个程序员都曾遇到过表示一个带有作为注意力焦点的子树的树的问题, "
      "其中焦点可以在树中左移, 右移, 上移, 下移. "
      "zipper是Huet为一种能够满足这种需求的精致数据结构取的巧妙的名字. "
   ...
   (P "纯粹应用性的编程范式的主要缺陷在于许多高效的算法都依赖于数据结构的破坏性操作, "
      ...
      "对于这种问题的一种著名解决方案是所谓的" (Em "函数式数组") " (Paulson, 1991). "
      "对于树而言, 这相当于以如下方法非破坏性地修改树中的出现, 即复制从树的根到这个出现的"
      (Em "路径") ". 当数据结构是只是局部于某个算法的对象时, 这被认为是可以忍受的, "
      "因为相较于复制整个树的朴素方法而言, 其只需要对数的代价. "
      "但是若数据结构表示了某种全局性的上下文, 例如文本编辑器的缓冲区, "
      "或者是一个证明系统的公理和引理数据库, 那么这种技术就是不可接受的了. "
      "在这个笔记里, 我们解释了一种简单的解决方案, 其中的树编辑完全是局部性的, "
      "数据上的柄 (handle) 不是树的原本的根, 而是在树中的当前位置.")
   (P "基本的想法是简单的: 树被从里翻到外, 就像手套一样, 在" (Em "路径结构")
      "里, 从根到当前位置的指针被反转了. [译注: 虽然说其实反转的顺序就是自然的, 类似于栈.] "
      "当前的" (Em "位置") "不仅保存了向下的当前子树, 也保存了向上的路径. "
      "所有的导航和修改原语都操作于位置结构上. "
      "在结构中上和下可以类比于关闭和打开一个拉链 (zipper), 故取此名.")
   (P "作者在设计一个结构编辑器的核心时想到了这种数据类型, "
      "这个结构编辑器是作为一个证明助手的结构管理器使用的. "
      "这个简单的想法肯定已被诸多程序员在无数场合下发明出来, "
      "而我呈现这个应该算是口口相传的东西的唯一理由在于其似乎还没有被发表过, "
      "更不用说为人熟知了.")
   (H2. "zipper数据结构")
   (P "基本的想法存在着诸多的变体. 首先让我们呈现一个版本, "
      "其适用于具有可变元数的匿名树结点的树, "
      "而树叶所注入的值来源于一个未加刻画的" (Em "item") "类型.")
   (H3. "树, 路径, 和位置")
   (P "我们假定存在一个类型参数" (Code "item")
      ", 其包含了我们想要层次性地操纵的元素; "
      "树结构不过就是层次性的列表, 其将树组织为section. "
      "例如, 在UNIX的文件系统中, item会是文件, 而section则是目录; "
      "而在一个文本编辑器里, item会是字符, 而两个层次的section分别"
      "将缓冲区表示为行的列表以及将行表示为字符的列表. "
      "推广这个概念至允许任意多个层次, 那么我们将得到层次性Turing机器的概念, "
      "其纸带位置 (tape position) 可能包含一个符号, "
      "或者是一个来源于更低层级的纸带.")
   (P "这里所呈现的算法均以具体的编程语言OCaml写成 (Leroy " (I "et al.")
      ", 1996). 这些代码很容易翻译为其他编程语言, 不论是否为函数式的, "
      "不论是否是惰性的.")
   (CodeB "type tree =
    Item of item
  | Section of tree list;;")
   (P "现在我们考虑" (Code "tree") "中的" (Code "path") ".")
   (CodeB "type path =
    Top
  | Node of tree list * path * tree list;;")
   (P "一个" (Code "path") "就像一条拉链 (zipper), "
      "其允许人们撕开树结构到一个特定的位置. 一个"
      (Code "Node(l,p,r)") "包含了其哥哥 (elder siblings) 的列表"
      (Code "l") " (自其中最年轻的起), 其父路径" (Code "p")
      ", 以及其弟弟 (younger siblings) 的列表" (Code "r")
      " (自其中最年长的起). "
      "[译注: 我怀疑原文作者将最年轻和最年长搞反了, 于是改成了现在这样, "
      "因为这里是越左越年长, 所以说" (Code "l")
      "的开头实际上应该是比当前焦点 (见后文) 年长的里面最年轻的, 而"
      (Code "r") "的开头实际上应该是比当前焦点年轻的里面最年长的.]")
   ((comment)
    "一个由路径所表示的树可以有兄弟树, 叔叔树, 曾叔叔树等等, "
    "但是其父是一个路径, 而非像通常的图编辑器 (graph editor) 里那样是一个树.")
   (CodeB "type location = Loc of tree * path;;")
   (P "一个" (Code "location") "由一个突出的" (Code "tree")
      " (其是当前的焦点) 和其" (Code "path") " (其代表了周围的上下文) 构成. "
      "注意到一个位置并不对应于树中的一个出现, "
      "而是一个指向某个特定子树与其周围上下文之间连接的弧的指针. "
      "[译注: 应该说是一种概念上的指针而非实际上的指针.]")
   ((example)
    "假定我们考虑parse算术表达式的树, 以字符串为item. 表达式"
    (&+ (&c* $a $b) (&c* $c $d)) "会被parse为树:"
    (CodeB "Section[Section[Item &quot;a&quot;; Item &quot;*&quot;; Item &quot;b&quot;];
        Item &quot;+&quot;;
        Section[Item &quot;c&quot;; Item &quot;*&quot;; Item &quot;d&quot;]];;")
    "树中第二个乘法符号的位置是 [译注: 我猜为什么这里没有以" (Code ";;")
    "作结, 是因为这是pretty print的输出而不是手写的程序]:"
    (CodeB "Loc(Item &quot;*&quot;,
    Node([Item &quot;c&quot;],
         ...
         [Item &quot;d&quot;]))"))
   (H3. "树中导航的原语")
   (CodeB "let go_left (Loc(t,p)) = match p with
...
let go_down (Loc(t,p)) = match t with
  ...
  | _ -> failwith &quot;down of empty&quot;;;")
   ((comment)
    "除了" (Code "go_up") "之外的导航原语都只需要常量时间, 而"
    (Code "go_up") "所消耗的时间正比于"
    (Code "list_length(left)") ".")
   (P "我们可以利用这些原语编写访问当前树的第" $n "个儿子的函数."
      (CodeB "let nth loc = nthrec
  ...
  | n -> if n>0 then go_right(nthrec (n-1))
   ...
   (H3. "改变, 插入, 和删除")
   (P "我们可以局部地改变当前位置的结构:")
   (CodeB "let change (Loc(_,p)) t = Loc(t,p);;")
   (P "向左或者向右插入是自然且廉价的:")
   (CodeB "let insert_right (Loc(t,p)) r = match p with
...
let insert_down (Loc(t,p)) t1 = match t with
  ...
  | Section(sons) -> Loc(t1,Node([],p,sons));;")
   (P "我们可能也想要实现一个删除原语. 我们可以选择可能的话就向右移动, "
      "否则就向左移动, 以及在空表的情形下向上移动.")
   (CodeB "let delete (Loc(_,p)) = match p with
  ...
  | Node([],up,[]) -> Loc(Section[],up);;")
   (P "我们应该注意到" (Code "delete") "并非一个简单的操作.")
   (P "我们相信以上的数据类型和操作足以用来以应用性但高效的方式"
      "编写一个结构编辑器的内核.")
   (H2. "基本想法的变体")
   (H3. "伤疤")
   (P "如果一个算法的某个频繁操作需要在树中先上再下到相同的位置, "
      "那么总是闭合section是很浪费时间的 (也浪费空间和垃圾回收时间, 等等). "
      "或许在结构中留下" (Q "伤疤") "会比较好, "
      "其允许直接访问之前记下来的已访问位置. "
      "因此, 我们将(非空的)section替换为记忆了一个树和其兄弟的三元组:")
   (CodeB "type memo_tree =
    Item of item
...
type memo_location = Loc of memo_tree * memo_path;;")
   (P "基于这些新的结构, 我们可以呈现简化了的上下操作.")
   (CodeB "let go_up_memo (Loc(t,p)) = match p with
    Top -> failwith &quot;up of top&quot;
