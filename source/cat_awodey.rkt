#lang racket
(provide thunk:cat_awodey.html)
(require SMathML "todo.rkt" "utils.rkt")
(define $CT (Mi "CT"))
(define $CT^* (&* $CT))
(define Σ* (&* Σ))
(define Δ* (&* Δ))
...
(define (restrict f A)
  (: f (_ $lv A)))
(define $+:id (Mi "+"))
(define zero-vec (vec 0 0))
(define (:FO position #:offset [offset zero-vec]
             #:width [width 100] #:height [height 30] . x*)
  (define pos
    (pt+ position offset))
  (keyword-apply
   ForeignObject
...
(define (:arrow-prop start end #:prop [prop 0.7])
  (define t (/ (- 1 prop) 2))
  (define start^ (pt-lerp start end t))
  (define end^ (pt-lerp start end (- 1 t)))
  (:arrow start^ end^))
...
(define UMP:free-monoid.svg
  (Svg
   ...
   (Defs arrow-head)
   (:FO (pt 0 0) (Em "在" Mon "之中:"))
   (:FO (pt 120 30) (free-monoid $A))
   (:FO (pt 360 30) $N)
   (set-dotted
    (:arrow-prop (pt+ (pt 120 30)
                      (vec 20 10))
                 (pt+ (pt 360 30)
                      (vec 20 10))))
   (:FO (pt-lerp (pt 120 30) (pt 360 30) 1/2)
        #:offset (vec 10 -10)
        (OverBar $f))
   (:FO (pt 0 60) (Em "在" Sets "之中:"))
   (:FO (pt 120 90)
        #:offset (vec -4 0)
   ...
   (:arrow-prop #:prop 0.85
                ...
                (pt+ (pt 360 90)
                     (vec 18 10)))
   (:FO (pt-lerp (pt 120 90) (pt 360 90) 1/2)
        #:offset (vec 8 -10)
        (forget (OverBar $f)))
   (:FO (pt-lerp (pt 120 90) (pt 120 280) 1/2)
        #:offset (vec 6 0)
        $i)
   (:FO (pt-lerp (pt 120 280) (pt 360 90) 1/2)
        #:offset (vec 18 10)
        $f)))
(define UMP:free-category.svg
  (Svg
   #:attr* '((width "480")
             ...
             (style "display: block; margin: auto;"))
   (Defs arrow-head)
   (:FO (pt 0 0) (Em "在" Cat "之中:"))
   (:FO (pt 120 30) (free-category $G))
   (:FO (pt 360 30) CatD)
   (set-dotted
    (:arrow-prop (pt+ (pt 120 30)
                      (vec 20 10))
   ...
   (:FO (pt-lerp (pt 120 30) (pt 360 30) 1/2)
        #:offset (vec 10 -10)
        (OverBar $h))
   (:FO (pt 0 60) #:width 120 (Em "在" Graphs "之中:"))
   (:FO (pt 120 90)
        #:offset (vec -4 0)
  ...
  (Svg
   ...
   (:arrow-prop
    ...
    (pt+ (pt 210 30)
         (vec 8 23)))
   (:FO
    (pt-lerp (pt 30 30) (pt 120 30) 1/2)
    #:offset (vec 4 -10)
    $compose)
   (:FO
    (pt-lerp (pt 120 30) (pt 210 30) 1/2)
    #:offset (vec 4 -1)
    $i)
   (:FO
    (pt-lerp (pt 120 30) (pt 210 30) 1/2)
...
(define set-product.svg
  (Svg
   ...
   (:arrow-prop (pt+ (pt 163 165)
                ...
                #:prop 0.65)
   (set-dotted
    (:arrow-prop (pt+ (pt 180 15)
                      (vec 3 5))
                 (pt+ (pt 180 165)
                      (vec 3 5))))
   (:FO (pt 92 80) $a)
   (:FO (pt 270 80) $b)
   (:FO (pt 190 90) (tu0 $a $b))
   (:FO (pt 95 175) $pi_1)
   (:FO (pt 250 175) $pi_2)))
...
(define (thunk:cat_awodey.html)
  ...
  (TnTmPrelude
      ...
      (Svg
       ...
       (:arrow-prop
        ...
        (pt+ (pt 210 130)
             (vec 8 23)))
       (:FO
        (pt-lerp (pt 30 130) (pt 120 130) 1/2)
        #:offset (vec 4 10)
        $compose)
       (:FO
        (pt-lerp (pt 120 130) (pt 210 130) 1/2)
        #:offset (vec 4 -1)
        $i)
       (:FO
        (pt-lerp (pt 120 130) (pt 210 130) 1/2)
       ...
       (:arrow-prop
        ...
        (pt+ (pt 210 130)
             (vec 10 9)))
       (:FO
        (pt-lerp (pt 30 30) (pt 30 130) 1/2)
        #:offset (vec -12 0)
        $F_2)
       (:FO
        (pt-lerp (pt 120 30) (pt 120 130) 1/2)
        #:offset (vec -12 0)
        $F_1)
       (:FO
        (pt-lerp (pt 210 30) (pt 210 130) 1/2)
        #:offset (vec 15 0)
        $F_0))
      "其中"
      (&= (ap $F_2 (tu0 $f $g))
      ...
      (Svg
       ...
       (:arrow-prop
        ...
        (pt+ (pt 150 150)
             (vec 10 9)))
       (:FO
        (pt-lerp (pt 30 30) (pt 150 30) 1/2)
        #:offset (vec 4 -12)
        $t)
       (:FO
        (pt-lerp (pt 30 30) (pt 150 30) 1/2)
        ...
        $s)
       (:FO
        (pt-lerp (pt 30 150) (pt 150 150) 1/2)
        #:offset (vec 4 -12)
        $t)
       (:FO
        (pt-lerp (pt 30 150) (pt 150 150) 1/2)
        ...
        $s)
       (:FO
        (pt-lerp (pt 30 30) (pt 30 150) 1/2)
        #:offset (vec -10 0)
        $h_1)
       (:FO
        (pt-lerp (pt 150 30) (pt 150 150) 1/2)
        #:offset (vec 14 0)
        $h_0))
   ...
   (H3. "练习")
   (H2. "群和范畴")
   (P "本章致力于群和范畴的各种各样的联系的其中一些. "
      "如果你已经了解了这里所讲述的基本的群论, "
      "那么本章将会为你提供一些关于你已经学到了的范畴构造的洞察; "
      "如果你还不知道群论, 那么你现在会将群论当作范畴论的应用进行学习. "
      "我们聚焦于范畴和群之间的关系的三个不同方面:"
      (Ol (Li "范畴中的群,")
          (Li "群的范畴,")
          (Li "群作为范畴.")))
   (H3. "范畴中的群")
   (P "我们已经看到, 群的概念是作为对于对象的自同构的抽象而产生的. "
      "在特定的具体的情形下, 一个群" $G
      "因而可能由对于某个范畴" CatC
      "中的某个对象" $X "的特定箭头"
      (Arrow $g $X $X) "构成, 即"
      (MB (&sube $G (Hom CatC $X $X)) ".")
      "但是, 抽象的群概念也可以被直接地描述为范畴中的对象, "
      "装备了特定的结构. 这种" (Q "范畴中的群")
      "的更为微妙的概念也被证明是相当有用的.")
   (P "令" CatC "是一个具有有限积的范畴. "
      CatC "中的群的概念本质上是对于" Sets
      "中的通常的群概念的推广.")
   ((Definition)
    CatC "中的一个" (Em "群")
    "由如下图所示的对象和箭头构成:"
    (Svg
     ...
     (:FO (pt 200 30) $G)
     (:FO (pt 204 200) $1)
     (:arrow (pt 80 38) (pt 187 38))
     (:arrow (pt 350 38) (pt 227 38))
     (:arrow (pt 207 185) (pt 207 50))
     (:FO (pt 125 15) $m)
     (:FO (pt 290 15) $i)
     (:FO (pt 190 110) $u))
    "其满足以下条件:"
    (Ol (Li $m "是结合性的, 即以下图表交换:"
   ...
   (H3. "练习")
   (H2. "极限和余极限")
   (P "在本章中, 我们首先简要讨论一些与已有定义相关的主题, 即子对象和拉回. "
      "这样做一方面是为了了解它们如何被使用, "
      "另一方面也是因为我们很快就需要用到这些材料. "
      "然后我们以更系统的方式展开, 定义极限的一般概念, "
      "它涵摄了我们迄今所遇到的许多特定的抽象刻画. "
      "当然, 还有一个对偶概念即余极限, 它同样有许多有趣的应用. "
      "在第6章简要考察另一个基本概念之后, 我们将进入可以称为"
      (Q "高阶范畴论") "的领域.")
   (H3. "子对象")
   (P "我们已经看到一个集合" $X "的每个子集" (&sube $U $X)
      "都作为一个等化子而出现, 而等化子总是单态射. "
   ...
   (H2. "自然性")
   (H2. "图表的范畴")
   (P "本章我们将会证明一个非常有用的技术性结果, 即米田引理, "
      "并将其应用于对集合值函子的范畴的研究, "
      "或者说" (Q "图表") "的范畴. "
      "米田引理或许是范畴论中最常使用的单个结果. "
      "其可以视为对于幺半群和偏序集上的某些简单结果的直接推广, "
      "尽管其有着大量深远的应用.")
   (H3. "集合值函子范畴")
   (P "我们将专注于具有形式"
      (MB (^ Sets CatC))
      "的特别函子范畴, 其中范畴" CatC
      "是局部小的. 因此, 其对象是集合值函子"
      (MB (Functor (&cm $F $G) CatC Sets))
      "有时也称为"
      (Q CatC "上的图表")
      ", 而箭头是自然变换"
      (MB (Natural (&cm $alpha $beta) $F $G) ".")
      "当" (&= CatC $P) ", 一个偏序集时, "
      "我们已经将这样的函子考虑为了"
      (Q "可变集合 (variable sets)")
      ", 即依赖于参数" (∈ $i $P)
      "的集合" $F_i ". 非偏序集" CatC
      "的一般情形也"
      )
   ...
   (P "本章代表了这本书的高潮, "
      "也是我们一直稳步推进所朝向的目标. "
      "伴随函子的概念由D. Kan在20世纪50年代首次发现, "
      "它将我们迄今所学的一切加以运用, "
      "从而统一并涵摄我们所遇到的所有不同的泛映射性质, "
      "从自由群到极限再到指数对象. 但更重要的是, "
      "它还捕捉了一种若没有范畴论的视角便无从察觉的重要数学现象. "
      "事实上, 我将提出一个公认具有挑衅性的主张: "
      "伴随性是一个具有根本逻辑和数学重要性的概念, "
      "而这一概念在数学的其他领域中未曾被捕捉到.")
   (P "范畴论最引人注目的许多应用都涉及伴随函子, "
      "而许多重要且基本的数学概念都是伴随函子的实例. "
      "因此, 它们共享所有伴随函子的一般行为和形式性质, "
      "并且在许多情况下, 仅凭这一事实就足以解释它们全部的本质特征.")
   (H3. "预备定义")
