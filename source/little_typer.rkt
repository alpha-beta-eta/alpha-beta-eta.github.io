#lang racket
(provide thunk:little_typer.html)
(require SMathML)
(define-entry*
  (tcomment "tcomment" "译注~a."))
(define marker0
  (Marker
   #:attr*
   '((id "arrow")
...
(define cong.svg
  (let ((X (pt 30 30))
    ...
    (Svg
     ...
     (:FO X $X)
     (:FO Y $Y)
     (:arrow X Y #:offset (vec 0 13))
     (:FO (pt+ ((lerp 1/2) X Y) (vec -9 -5)) $f)
     ...
     (set-attr* (:FO B (Code "(= " $Y
                             ...
                             $f " " $to "))"))
                'width "200")
     (:arrow (pt 170 40) (pt 170 140)
             #:prop 0.6)
     (set-attr* (:FO (pt 190 55)
                     (CodeB "(cong " (Frame "     ") " " $f ")"))
                'height "50" 'width "200")
     )))
(define (thunk:little_typer.html)
  (TnTmPrelude
