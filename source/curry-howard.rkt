#lang racket
(provide thunk:curry-howard.html)
(require (except-in
          SMathML
          format-section Definition Remark Theorem Example
          Convention Lemma Notation Proposition)
         "curry-howard-utils.rkt")
(define (:arrow start end #:prop [prop 0.8] #:offset [vec vec:zero])
  ...
  (:arr p1 p2))
(define (thunk:curry-howard.html)
  (parameterize ([stroke-width "1.2px"])
    (TnTmPrelude
     ...
     (&label
      (Svg
       ...
       (Defs marker0)
       (:FO (pt 115 0) (B "证明者1"))
       (:arrow (pt 160 0) (pt 160 60) #:offset (vec 0 15) #:prop 0.5)
       (:FO (pt 115 60) (B "怀疑者1"))
       (:arrow (pt 160 60) (pt 160 120) #:offset (vec 0 15) #:prop 0.5)
       (:FO (pt 115 120) (B "证明者2"))
       (:arrow (pt 160 120) (pt 35 180) #:offset (vec 15 15) #:prop 0.5)
       (:FO (pt 35 180) (B "怀疑者2"))
       (:arrow (pt 35 180) (pt 35 260) #:offset (vec 45 15) #:prop 0.5)
       (:FO (pt 35 260) (B "证明者3"))
       (:arrow (pt 130 120) (pt 240 180) #:offset (vec 15 15) #:prop 0.5)
       (:FO (pt 195 180) (B "怀疑者2'"))
       (:arrow (pt 195 180) (pt 195 260) #:offset (vec 45 15) #:prop 0.5)
       (:FO (pt 195 260) (B "证明者3'")))
      "图4.2: 证明者策略")
     ...
     (&label;tricky and delicate
      (Svg
       ...
       (:FO (pt 115 0) (B "P1"))
       (:arrow (pt 160 0) (pt 160 60) #:offset (vec 0 15) #:prop 0.5)
       (:FO #:width "300" (pt 20 60) (B "S1") " "
            (tlam $x (&-> (@-> $p $p) (@-> $q $r $q) $s) $...))
       (:arrow (pt 160 60) (pt 160 120) #:offset (vec 0 15) #:prop 0.5)
       (:FO (pt 115 120) (B "P2") " " (ap* $x $... $...))
       (:arrow (pt 160 120) (pt 35 180) #:offset (vec 15 15) #:prop 0.5)
       (:FO (pt 35 180) (B "S2") " " (tlam $y $p $...))
       (:arrow (pt 35 180) (pt 35 260) #:offset (vec 45 15) #:prop 0.5)
       (:FO (pt 35 260) (B "P3") " " $y)
       (:arrow (pt 130 120) (pt 240 180) #:offset (vec 15 15) #:prop 0.5)
       (:FO #:width "120" (pt 195 180) (B "S2'") " "
            (tlam $u $q (tlam $v $r $...)))
       (:arrow (pt 195 180) (pt 195 260) #:offset (vec 45 15) #:prop 0.5)
       (:FO (pt 195 260) (B "P3'") " " $u))
      "图4.3: 由策略得到构造")
     ...
     (P "之前的章节中我们重点关注了自然演绎证明和" $lambda "项之间的Curry-Howard同构. "
        ...
        "另外两种基础概念之间的对应, 即" (Em "Hilbert风格")
        "的证明和" (Em "组合子逻辑") "的项.")
     (P $lambda "演算的一个相当不便的方面在于绑定变量所扮演的角色, "
        "特别是当我们将其与替换联系起来的时候. "
        "当我们处理自由变量和绑定变量的概念时, "
        "类似的问题也总是出现. "
        "为了避免这些问题, Sch&ouml;finkel和Curry引入了组合子逻辑, "
        "这是一种没有绑定变量的系统, 在诸多方面等价于" $lambda
        "演算. 和" $lambda "抽象不同的是, 这种演算只处理常量的应用.")
     (P "从Curry-Howard同构的角度来看, "
        "这种改变对应于拒绝implication的引入规则, 然后将其替换以公理. "
        "结果是一个Hilbert风格的证明系统, "
        "这是表达形式证明的概念的最传统方式.")
     (P "组合子逻辑能够表达" $lambda "演算这一事实 "
        "(由Sch&ouml;finkel和Curry发现) "
        "可以直接地翻译为Hilbert风格可证明性的一个非常基本的性质, 即"
        (Em "演绎定理(deduction theorem)")
        ", 由Herbrand所发现. 因此, Curry-Howard同构"
        "又一次在两个独立发展的数学世界之间建立了桥梁.")
