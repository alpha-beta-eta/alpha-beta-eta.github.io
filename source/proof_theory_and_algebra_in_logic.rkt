#lang racket
(provide thunk:proof_theory_and_algebra_in_logic.html)
(require SMathML)
...
(define (thunk:proof_theory_and_algebra_in_logic.html)
  (parameterize ([separate-class? #t]
                 [parameter:format-num format-num])
  (TnTmPrelude
   #:title "证明论和逻辑代数"
   #:css "styles.css"
   (H1. "证明论和逻辑代数")
   ...
   (P "在例子6.5和练习6.6中, 我们表明了任意一个集合" $C
      "的幂集" (powset $C) "相对于并" $union "和交"
      $cap "构成了一个分配格"
      (tupa0 (powset $C) $union $cap)
      ", 其中" $empty "是最小元. 对于" $C
      "的每个子集" $X ", 令" (&- $X) "是" $X
      "的补. 对于" (&sube (&cm $X $Y) $C) ", 定义"
      (&= (&-> $X $Y) (&union (&- $X) $Y)) ". 那么, "
      (&= (powset $C:bold)
          (tupa0 (powset $C) $union $cap $->:id $empty))
   ...
   (P "令" $C "是任意一个具有" $m "个元素的有限集合, 例如"
      ...
      ". {译注: 这默认了当" (&!= $i $j) "时, "
      (&!= $a_i $a_j) ".} 那么, 幂集布尔代数"
      (powset $C:bold) "是一个" $2^m
      "-值布尔代数. 如果" $D "是另一个具有" $m
      "个元素的集合, 那么显然" (powset $D:script)
      "同构于" (powset $C:bold)
      ". 我们注意到"
      )
   (H4 "无限布尔代数")
