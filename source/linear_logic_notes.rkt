#lang racket
(provide thunk:linear_logic_notes.html)
(require SMathML)
(define $○
  (set-compact (Mo "○")))
(define (&○ M) (ap $○ M))
...
(define (lax A) (cur0 A))
(define (thunk:linear_logic_notes.html)
  (TnTmPrelude
   #:title "线性逻辑笔记"
   #:css "styles.css"
   (H1. "线性逻辑笔记")
   ...
   (H2. "合一")
   (H2. "monad风格逻辑编程")
   (P "我们已经详细讨论过前向链和后向链, 在本讲中, "
      "我们将探讨如何把二者有效地结合起来. "
      "其核心思想是利用monad将前向链接与后向链接分离开来. "
      "monad起源于范畴论, 并已在函数式编程中得到成功应用. "
      "在逻辑编程中, 其当前形态最早是在"
      "LolliMon语言 [LPPW05] 中被提出的.")
   (H3. "monad")
   (P "我们引入一种新的类型" (lax $A)
      ", 有时在逻辑学中记为" (&○ $A)
      ", 而在范畴论中记为" (ap $T $A)
