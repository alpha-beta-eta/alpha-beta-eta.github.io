...
(define (thunk:hott.html)
  (TnTmPrelude
   ...
   (P "另一种等价的记号是"
      ...
      "中用空白" (Q $dummy) "代替变量以指称隐式的" $lambda
      "抽象. 例如, " (appl $g $x $dummy) "是另一种书写"
   ...
   (H3 "依赖函数类型 (" $Pi:normal "类型)" #:id "sec1-4")
   ...
   (P "给定类型" (&: (&cm $A $B) Universe) ", 我们可以引入类型"
      ...
      ". 我们也引入了一个零元积类型, 其被称为" (B "单元类型 (unit type)")
      ...
      "并将它们收集起来以形成笛卡尔积.")
   ((Remark)
    "在类型论中引入新的种类的类型存在着一般的模式. "
    "我们已经在第" (Ref "sec1-2") "节和第" (Ref "sec1-4")
    "节看到过了这种模式, 故有必要强调其一般的形式. "
    "为了描述一个类型, 我们需要刻画:"
    (Ol #:attr* '((type "i"))
        (Li "如何形成 (form) 这种新的类型, 通过"
            (B "形成规则 (formation rule)")
            ". (例如, 当" $A "是一个类型且" $B
            "是一个类型时, 我们可以形成函数类型"
            (&-> $A $B) ". 当" $A "时一个类型且"
            (app $B $x) "对于(每个)" (&: $x $A)
            ...
            (Pi $x $A (app $B $x)) ".")
        (Li "如何构造这个类型的元素. "
            "这些被称为类型的" (B "构造子 (constructor)")
            "或者说" (B "引入规则 (introduction rule)")
            ". (例如, 一个函数类型有着一个构造子, 即"
            $lambda "抽象. 回忆一下, 诸如"
            (&:≡ (app $f $x) (&i* $2 $x))
            "这样的直接定义可以被等价地重述为" $lambda "抽象"
            (&:≡ $f (lam $x (&i* $2 $x))) ".)")
        (Li "如何使用这个类型的元素. 这些被称为类型的"
            (B "消去子 (eliminator)") "或者"
            (B "消去规则 (elimination rule)")
            ". (例如, 函数类型拥有一个消去子, 即函数应用.)")
        (Li (B "计算规则 (computation rule)")
            ", 其描述了一个消去子该如何作用于一个构造子之上. "
            "(例如, 对于函数而言, 计算规则陈述了"
            (app (@lam $x $Phi:normal) $a)
            "判断相等于将" $Phi:normal "中的" $x
            "替换为" $a ".)")
        (Li "可选的" (B "唯一性规则 (uniqueness rule)")
            ", 其表达了进这个类型或者出这个类型的映射的唯一性. "
            "对于某些类型而言, "
            )
        )
    )
   ...
   (P "同伦类型论的中心新想法在于类型可以被视为同伦论中的空间, "
      "或者范畴论中的高维群胚.")
   (P "我们从简要总结同伦论和高维范畴论之间的关联开始. "
      "在经典同伦论之中, 一个空间" $X
      "是装备了一个拓扑的点集, 而点" $x "和" $y
      "之间的一条路径则由一个连续映射"
      (func $p (|[]| $0 $1) $X)
      "表示, 其中" (&= (app $p $0) $x)
      "而" (&= (app $p $1) $y)
      ". 这个函数可以想成是在每一" (Q "时刻")
      "给出了" $X "中的一个点. 对于诸多目的而言, "
      "路径的严格相等性 (意即逐点相等的函数) "
      "作为概念而言有点太过苛刻. "
      "例如, 我们可以定义道路连接操作 "
      "(如果" $p "是从" $x "到" $y "的一条路径, "
      $q "是从" $y "到" $z "的一条路径, 那么连接"
      (&d* $p $q) "是从" $x "到" $z
      "的一条路径) 和逆运算 (" (inv $p)
      "是一条从" $y "到" $x "的路径). "
      "然而, 存在着自然的等式对于严格的相等性而言并不成立: "
      "例如, 路径" (&d* $p (inv $p))
      " (当时间从" $0 "走到" $1 "时, 其从"
      $x "走到" $y ", 然后又以相同的线路返回) "
      "并不严格等于恒等路径 (其在所有时刻都呆在" $x "不动).")
   (P "补救办法在于考虑一种更为宽泛的路径相等概念, 其被称为"
      (Em "同伦(homotopy)") ". 一对连续映射"
      (func $f $X_1 $X_2) "和" (func $g $X_1 $X_2)
      "之间的一个同伦是一个连续映射"
      (func $H (&c* $X_1 (|[]| $0 $1)) $X_2)
      ", 其满足" (&= (appl $H $x $0) (app $f $x))
      "而" (&= (appl $H $x $1) (app $g $x))
      ". 在" $p "和" $q "都是从" $x "到" $y
      "的路径的特定情形之下, 一个同伦是一个连续映射"
      (func $H (&c* (|[]| $0 $1) (|[]| $0 $1)) $X)
      ", 其满足对于所有的" (∈ $s (|[]| $0 $1))
      "都有" (&= (appl $H $s $0) (app $p $s))
      "而" (&= (appl $H $s $1) (app $q $s))
      ". 在这种情形下我们也要求对于每个"
      (∈ $t (|[]| $0 $1)) "都有"
      (&= (appl $H $0 $t) $x) "而"
      (&= (appl $H $1 $t) $y)
      ", 于是对于每个" $t "而言函数"
      (appl $H $dummy $t) "又是从"
      $x "到" $y "的一条路径; 这种同伦被人称为"
      (Em "端点保持的(endpoint-preserving)")
      ", 或者说" (Em "rel endpoints")
      ". 在简单的情况下, 我们可以将正方形"
      (&c* (|[]| $0 $1) (|[]| $0 $1))
      "在" $H "下的像想成是"
      (Q "填满了" $p "和" $q "之间的空间")
      ", 尽管对于一般的" $X "而言这并无真切的意义; "
      "我们最好将" $H "想成是从" $p "到" $q
      "的不移动端点的连续形变. 既然"
      (&c* (|[]| $0 $1) (|[]| $0 $1))
      "是" $2 "维的, 我们也将" $H "说成是"
      $2 "维的" (Em "路径之间的路径") ".")
   (P "例如, 因为" (&d* $p (inv $p))
      "沿着相同的路线来回, 所以说你知道你可以将"
      (&d* $p (inv $p)) "连续地收缩为恒等路径"
      "&mdash;&mdash;"
      )
   (H3 "类型是高阶群胚")
   (H3 "函数是函子")
