#lang racket
(provide thunk:denotational_semantics_notes.html)
(require SMathML)
...
(define vec:down (vec 0 1))
(define vec:up (vec 0 -1))
(define vec:left (vec -1 0))
(define vec:right (vec 1 0))
(define offset:down (vec -7 -4))
(define offset:up (vec -7 -13))
(define offset:left (vec -14 -9))
(define offset:right (vec -2 -9))
...
(define (:arrow start end #:prop [prop 0.8])
  (define t (/ (- 1 prop) 2))
  (define p1 (pt-lerp start end t))
  (define p2 (pt-lerp start end (- 1 t)))
  (:arr p1 p2))
(define $LUB (Mo "&bigsqcup;"))
(define LUB (make-bigop $LUB))
(define $fix (Mi "fix"))
...
(define &p-> &RightVector)
(define @p-> @RightVector)
(define (thunk:denotational_semantics_notes.html)
  (TmPrelude
   #:title "指称语义学讲义"
   #:css "styles.css"
   (H1 "指称语义学讲义")
   ...
   ((example #:n "4")
    ...
    (Svg
     ...
     (:dot 260 120)
     (:dot 160 30)
     (:arrow (pt 60 120) (pt 160 30) #:prop 0.7)
     (:arrow (pt 260 120) (pt 160 30) #:prop 0.7))
   ...
   (&label
    (Svg
     #:attr* '((width "500")
               ...
               (style "display: block; margin: auto;"))
     (Defs marker0)
     (:FO (pt 30 30) "&quot;扁平自然数集&quot;" (_ $NN $bottom) ":")
     (:FO (pt 60 60) $0)
     (:FO (pt 120 60) $1)
     (:FO (pt 180 60) $2)
     (:FO (pt 240 60) $..c)
     ...
     (:FO (pt 360 60) (&+ $n $1))
     (:FO (pt 450 60) $..c)
     (:FO (pt 240 120) $bottom)
     (:arrow (pt 240 130) (pt 60 80))
     (:arrow (pt 240 130) (pt 120 80))
     (:arrow (pt 240 130) (pt 180 80))
     (:arrow (pt 240 130) (pt 300 80))
     (:arrow (pt 240 130) (pt 360 80))
     (:FO (pt 30 150) "&quot;垂直自然数集&quot;" $Omega:normal ":")
     (:FO (pt 240 180) $omega)
     (:FO (pt 225 220) (&+ $n $1))
     (:FO (pt 240 260) $n)
     (:FO (pt 240 300) $2)
     (:FO (pt 240 340) $1)
     (:FO (pt 240 380) $0)
     (:arrow (pt 237 385) (pt 237 364))
     (:arrow (pt 237 345) (pt 237 324))
     (:FO (pt 243 280) $..v)
     (:arrow (pt 237 265) (pt 237 244))
     (:FO (pt 243 200) $..v))
    "图1")
   ...
   (P "自然数的扁平domain " (_ $NN $bottom)
      ...
      (Svg
       ...
       (:FO (pt 60 30) 'true)
       (:FO (pt 260 30) 'false)
       (:FO (pt 164 120) $bottom)
       (:arrow (pt 164 134) (pt 60 48))
       (:arrow (pt 164 134) (pt 275 48)))
