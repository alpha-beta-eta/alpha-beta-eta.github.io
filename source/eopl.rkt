...
(define eopl.html
  (Prelude
   ...
   (P "练习5.30解答. 当然了, 我不会全然按照题目的要求完成. 首先, 我们先给出句法."
      (CodeB "&lt;exp> ::= &lt;var>
       |  (proc (&lt;var>) &lt;exp>)
       |  (&lt;exp> &lt;exp>)")
      "之所以用" (Code "proc") "而不是" (Code "lambda")
      ", 是为了避免让人误以为这是lambda演算. 然后, 我们给出实现."
      (CodeB "(define EXP (void))
(define ENV (void))
(define VAL (void))
(define PRO (void))
(define CTX (void))
...
(define (RUN exp)
  ...
  (EVAL))")
      "注记. 动态作用域有着动态窗口期, "
      "也就是说, 对于形式参数的绑定恰在该过程返回时被弹出. "
      "在某种意义上, 动态窗口期是一种静态性质.")
   (P "练习5.53. 我仍然没有完全按照题目要求完成. "
      "现在过程" (Code "interp") ", "
      (Code "apply-ctx") ", " (Code "apply-closure")
      "都会多接受一个名为" (Code "tid")
...
(define run
  (case-lambda
    ...
    ((ticks exp)
     ...
     ((interp (new-thread-id))
      exp (empty-env) '(end-main-thread-ctx)))))"))
   (P "练习5.55. 这里的答案基于笔者之前写下的练习5.53的答案, "
      "以下未出现的定义就是和5.53保持一致."
      (CodeB "(define (make-scheduler name ticks)
  (define queue (void))
  (define time (void))
  (define value (void))
  ...
  (lambda (msg . arg*)
    (case msg
      ...
      ((set-ticks) (apply set-ticks arg*))
      (else (error name &quot;unknown message ~s&quot; msg)))))")
      (CodeB "(define ((apply-ctx tid) ctx val)
  (cond ((global-scheduler 'expired?)
         (global-scheduler
          'add-thread
          (lambda ()
        ...
        (else
         ...
         (match ctx
           ...
           ((end-subthread-ctx)
            ...
            (global-scheduler
             'run-next-thread))))))")
      (CodeB "(define ((interp tid) exp env ctx)
  (match exp
    (,int
     (guard (integer? int))
     ((apply-ctx tid) ctx int))
